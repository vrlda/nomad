...
#[test]
fn test_partitioned_network_cookies_are_isolated_by_top_level_site() {
    let mut storage = CookieStorage::new(5);
    let resource = ServoUrl::parse("https://cdn.example/resource").unwrap();
    let first_party = ServoUrl::parse("https://first.example/page").unwrap();
    let second_party = ServoUrl::parse("https://second.example/page").unwrap();
    let source = CookieSource::HTTP;

    let first_cookie = ServoCookie::new_wrapped(
        cookie::Cookie::parse("session=first; Secure").unwrap(),
        &resource,
        source,
    )
    .unwrap();
    storage.push_in_partition(first_cookie, &resource, &first_party, source);

    assert_eq!(
        storage.cookies_for_url_in_partition(&resource, &first_party, source),
        Some("session=first".to_owned())
    );
    assert_eq!(
        storage.cookies_for_url_in_partition(&resource, &second_party, source),
        None
    );

    let second_cookie = ServoCookie::new_wrapped(
        cookie::Cookie::parse("session=second; Secure").unwrap(),
        &resource,
        source,
    )
    .unwrap();
    storage.push_in_partition(second_cookie, &resource, &second_party, source);
    assert_eq!(
        storage.cookies_for_url_in_partition(&resource, &second_party, source),
