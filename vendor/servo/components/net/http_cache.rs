...
impl CacheKey {
    ...
    pub fn new(request: &Request) -> CacheKey {
        ...
        let partition_url = request.referrer.to_url().unwrap_or(&url);
        Self::from_partition_url(partition_site_key(partition_url), url)
    }

    /// Create a cache-key from a URL and its top-level-site partition.
    pub fn from_partition_url(partition: impl Into<String>, url: ServoUrl) -> CacheKey {
        let partition = partition.into();
        let serialized = serialize_key(&partition, &url);
        CacheKey {
            partition,
            url,
            serialized,
    ...
    pub fn from_url(url: ServoUrl) -> CacheKey {
        Self::from_partition_url(String::new(), url)
    }

    /// Return the partition used for this cache key.
    pub fn partition(&self) -> &str {
        &self.partition
    }
}

fn serialize_key(partition: &str, url: &ServoUrl) -> String {
    if partition.is_empty() {
        return url.as_str().to_owned();
    }
    format!("{partition}\u{1f}{}", url.as_str())
}

/// Derive the HTTP cache partition from the registered top-level site.
pub(crate) fn partition_site_key(url: &ServoUrl) -> String {
    url.host_str()
        .map(reg_suffix)
...
impl HttpCache {
    ...
    pub(crate) async fn invalidate_related_urls(
        ...
        for header_name in &[header::LOCATION, header::CONTENT_LOCATION] {
            ...
            {
                let location_key =
                    CacheKey::from_partition_url(skip_key.partition().to_owned(), location_url);
...
#[cfg(test)]
mod partition_tests {
    use super::{CacheKey, partition_site_key};
    use servo_url::ServoUrl;

    #[test]
    fn cache_key_keeps_same_resource_separate_by_top_level_site() {
        let url = ServoUrl::parse("https://cdn.example.test/app.js").unwrap();
        let first = CacheKey::from_partition_url("first.example", url.clone());
        let second = CacheKey::from_partition_url("second.example", url);
    ...
    #[test]
    fn partition_site_key_uses_registered_domain() {
        let url = ServoUrl::parse("https://sub.first.example.co.uk/page").unwrap();

        assert_eq!(partition_site_key(&url), "example.co.uk");
    }
}
