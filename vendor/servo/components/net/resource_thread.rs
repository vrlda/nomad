...
use net_traits::request::{
    Destination, PreloadEntry, PreloadId, RequestBuilder, RequestId, ServiceWorkersMode,
};
...
impl ResourceChannelManager {
    ...
    fn process_msg(
        ...
        match msg {
            ...
            CoreResourceMsg::DeleteCookieAsync(cookie_store_id, url, partition_url, name) => {
                let mut cookie_jar = http_state.cookie_jar.write();
                if pref!(network_nomad_partition_storage) {
                    cookie_jar.delete_cookie_with_name_in_partition(&url, &partition_url, name);
                } else {
                    cookie_jar.delete_cookie_with_name(&url, name);
                }
            ...
            CoreResourceMsg::SetCookiesForUrl(request, partition_url, cookies, source) => {
                for cookie in cookies {
                    self.resource_manager.set_cookie_for_url_in_partition(
                        &request,
                        &partition_url,
                        cookie.into_inner(),
                        source,
            ...
            CoreResourceMsg::GetCookieStringForUrl(url, partition_url, consumer, source) => {
                let mut cookie_jar = http_state.cookie_jar.write();
                if pref!(network_nomad_partition_storage) {
                    cookie_jar.remove_expired_cookies_for_url_in_partition(&url, &partition_url);
                    consumer.send_or_ignore(cookie_jar.cookies_for_url_in_partition(
                        &url,
                        &partition_url,
                        source,
                    ));
            ...
            CoreResourceMsg::GetCookieDataForUrlAsync(
                cookie_store_id,
                url,
                partition_url,
                name,
                ...
                let mut cookie_jar = http_state.cookie_jar.write();
                let cookie = if pref!(network_nomad_partition_storage) {
                    cookie_jar.remove_expired_cookies_for_url_in_partition(&url, &partition_url);
                    cookie_jar
                        .cookies_data_for_url_in_partition(
                            &url,
                            &partition_url,
                            CookieSource::NonHTTP,
                        )
                        .filter(|cookie| name.as_ref().is_none_or(|name| cookie.name() == name))
                        .map(Serde)
                        .next()
                } else {
                    cookie_jar.remove_expired_cookies_for_url(&url);
            ...
            CoreResourceMsg::GetAllCookieDataForUrlAsync(
                cookie_store_id,
                url,
                partition_url,
                name,
                ...
                let mut cookie_jar = http_state.cookie_jar.write();
                let cookies = if pref!(network_nomad_partition_storage) {
                    cookie_jar.remove_expired_cookies_for_url_in_partition(&url, &partition_url);
                    cookie_jar
                        .cookies_data_for_url_in_partition(
                            &url,
...
impl CoreResourceManager {
    ...
    fn set_cookie_for_url_in_partition(
        &mut self,
        request: &ServoUrl,
        ...
        if let Some(cookie) = ServoCookie::new_wrapped(cookie, request, source) {
            let mut cookie_jar = http_state.cookie_jar.write();
            if pref!(network_nomad_partition_storage) {
                cookie_jar.push_in_partition(cookie, request, partition_url, source);
            } else {
                cookie_jar.push(cookie, request, source);
            }
        }
    ...
    fn fetch<Target: 'static + FetchTaskTarget + Send>(
        ...
        let navigation_preload_request = request_builder.clone();
        let request = request_builder.build();
        let url = request.current_url();
        let service_worker_manager = self.sw_managers.get(&url.origin()).cloned();
        ...
        spawn_task(async move {
            ...
            if request.service_workers_mode == ServiceWorkersMode::All
                && let Some(service_worker_manager) = service_worker_manager
            {
                let (response_sender, response_receiver) = ipc::channel().unwrap();
                let mediator = CustomResponseMediator {
                    response_chan: response_sender,
                    load_url: request.current_url(),
                    request: navigation_preload_request,
                };

                if service_worker_manager.send(mediator).is_ok() {
                    let custom_response = tokio::time::timeout(
                    ...
                    if let Some(custom_response) = custom_response {
                        ...
                        response.headers = custom_response.headers;
                        response.status = HttpStatus::new_raw(
                            custom_response.raw_status.0.as_u16(),
                            custom_response.raw_status.1.into_bytes(),
                        );
                        *response.body.lock() = ResponseBody::Done(custom_response.body);

                        sender.process_response(&request, &response);
                        let body = match &*response.body.lock() {
                            ResponseBody::Done(body) => Some(body.clone()),
                            ResponseBody::Empty | ResponseBody::Receiving(_) => None,
                        };
                        if let Some(body) = body.filter(|body| !body.is_empty()) {
                            sender.process_response_length_hint(&request, body.len());
                            sender.process_response_chunk(&request, body);
                        }
