...
impl From<&Row<'_>> for DiskCacheMetadata {
    fn from(row: &Row) -> Self {
        ...
        let key = if let Some((partition, url)) = s.split_once('\u{1f}') {
            CacheKey::from_partition_url(partition.to_owned(), ServoUrl::parse(url).unwrap())
        } else {
            CacheKey::from_url(ServoUrl::parse(&s).unwrap())
        };
        Self {
            key,
