...
fn set_request_cookies(
    ...
    let cookie_list = if pref!(network_nomad_partition_storage) {
        ...
        cookie_jar.cookies_for_url_in_partition(url, partition_url, CookieSource::HTTP)
    } else {
        cookie_jar.remove_expired_cookies_for_url(url);
        cookie_jar.cookies_for_url(url, CookieSource::HTTP)
    };
...
fn set_cookies_from_headers(
    ...
    for cookie in headers.get_all(header::SET_COOKIE) {
        ...
        if let Ok(cookie_str) = std::str::from_utf8(cookie_bytes) {
            set_cookie_for_url(cookie_jar, url, partition_url, cookie_str);
        }
    }
}

fn is_third_party_cookie_request(request: &Request) -> bool {
    if matches!(&request.destination, Destination::Document) {
        return false;
    }
    let Some(referrer) = request.referrer.to_url() else {
        return false;
    };
    !is_same_site(&referrer.origin(), &request.current_url().origin())
}
...
async fn http_network_or_cache_fetch(
    ...
    if include_credentials {
        ...
        if !pref!(network_nomad_block_third_party_cookies)
            || !is_third_party_cookie_request(http_request)
        {
            let partition_url = http_request
                .referrer
                .to_url()
                .cloned()
...
async fn http_network_fetch(
    ...
    *res_body.lock() = ResponseBody::Receiving(vec![]);
    ...
    // Step 15
    if credentials_flag
        && (!pref!(network_nomad_block_third_party_cookies)
            || !is_third_party_cookie_request(request))
    {
