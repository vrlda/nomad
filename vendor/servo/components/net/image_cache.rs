        ...
        parse_svg_document_in_memory(bytes, usvg_options)
            ...
            .map(|svg_tree| {
                DecodedImage::Vector(VectorImageData {
                    ...
                    root_aspect_ratio: SvgAspectRatio::from_document(bytes),
                    cors_status: cors,
                })
            })
    } else {
...
struct VectorImageData {
    ...
    root_aspect_ratio: SvgAspectRatio,
...
impl Default for SvgAspectRatio {
    fn default() -> Self {
        Self {
            align: SvgAlign::MidMid,
            slice: false,
        }
    }
}
...
impl SvgAspectRatio {
    fn from_document(bytes: &[u8]) -> Self {
        static ATTRIBUTE: LazyLock<Regex> = LazyLock::new(|| {
            Regex::new(r#"(?is)<svg\b[^>]*\bpreserveAspectRatio\s*=\s*(?:\"([^\"]*)\"|'([^']*)')"#)
                .expect("valid preserveAspectRatio regex")
        });

        let Some(value) = std::str::from_utf8(bytes).ok().and_then(|document| {
            let captures = ATTRIBUTE.captures(document)?;
            captures
                .get(1)
        ...
        let alignment = if first.eq_ignore_ascii_case("defer") {
            tokens.next().unwrap_or_default()
        } else {
            first
        };
        let align = match alignment.to_ascii_lowercase().as_str() {
            "none" => SvgAlign::None,
            "xminymin" => SvgAlign::MinMin,
            "xmidymin" => SvgAlign::MidMin,
            "xmaxymin" => SvgAlign::MaxMin,
            "xminymid" => SvgAlign::MinMid,
            "xmidymid" => SvgAlign::MidMid,
            "xmaxymid" => SvgAlign::MaxMid,
            "xminymax" => SvgAlign::MinMax,
            "xmidymax" => SvgAlign::MidMax,
            "xmaxymax" => SvgAlign::MaxMax,
            _ => return Self::default(),
        };

        Self {
            align,
            slice: tokens.any(|token| token.eq_ignore_ascii_case("slice")),
        }
    }

    fn transform(
        self,
        source: tiny_skia::IntSize,
        destination: tiny_skia::IntSize,
    ) -> tiny_skia::Transform {
        let sx = destination.width() as f32 / source.width() as f32;
        let sy = destination.height() as f32 / source.height() as f32;
        if matches!(self.align, SvgAlign::None) {
            return tiny_skia::Transform::from_scale(sx, sy);
        }

        let scale = if self.slice { sx.max(sy) } else { sx.min(sy) };
        let spare_x = destination.width() as f32 - source.width() as f32 * scale;
        let spare_y = destination.height() as f32 - source.height() as f32 * scale;
        let x_factor = match self.align {
            SvgAlign::MinMin | SvgAlign::MinMid | SvgAlign::MinMax => 0.0,
            SvgAlign::MidMin | SvgAlign::MidMid | SvgAlign::MidMax => 0.5,
            SvgAlign::MaxMin | SvgAlign::MaxMid | SvgAlign::MaxMax => 1.0,
            SvgAlign::None => unreachable!(),
        };
        let y_factor = match self.align {
            SvgAlign::MinMin | SvgAlign::MidMin | SvgAlign::MaxMin => 0.0,
            SvgAlign::MinMid | SvgAlign::MidMid | SvgAlign::MaxMid => 0.5,
            SvgAlign::MinMax | SvgAlign::MidMax | SvgAlign::MaxMax => 1.0,
            SvgAlign::None => unreachable!(),
        };
...
impl ImageCache for ImageCacheImpl {
    ...
    fn rasterize_vector_image(
        ...
        self.thread_pool.spawn(move || {
            // A CSS-sized SVG establishes a new viewport. Map the root viewBox
            // into it using preserveAspectRatio. Scaling the axes independently
            // distorts every inline and external SVG whose CSS box and natural
            // size have different aspect ratios.
            let transform = vector_image
                .root_aspect_ratio
                .transform(natural_size, tinyskia_requested_size);
