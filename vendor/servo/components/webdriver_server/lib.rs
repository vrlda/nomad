...
/// Identity and embedder-owned capabilities returned from WebDriver sessions.
///
/// The standard WebDriver capabilities identify the engine through
/// `browserName` and `browserVersion`. Embedder-specific information belongs in
/// vendor-namespaced capabilities so clients can distinguish a Nomad session
/// from a standalone Servo session without relying on user-agent parsing.
#[derive(Clone, Debug)]
pub struct WebDriverIdentity {
    pub browser_name: String,
    pub browser_version: String,
    pub extra_capabilities: BTreeMap<String, Value>,
}

impl WebDriverIdentity {
    #[must_use]
    pub fn new(browser_name: impl Into<String>, browser_version: impl Into<String>) -> Self {
        Self {
            browser_name: browser_name.into(),
            browser_version: browser_version.into(),
            extra_capabilities: BTreeMap::new(),
        }
    }

    #[must_use]
    pub fn with_capability(mut self, name: impl Into<String>, value: Value) -> Self {
        self.extra_capabilities.insert(name.into(), value);
        self
    }

    fn servo() -> Self {
        Self::new("servo", "0.0.1")
    }
}
...
/// Starts the WebDriver HTTP server on an explicit address.
///
/// The default [`start_server`] entry point intentionally binds to loopback.
/// A WebDriver endpoint is a privileged browser-control surface and must not
/// become reachable from the network unless an embedder makes that decision
/// explicitly.
pub fn start_server_on(
    ...
    start_server_on_with_identity(
        address,
        embedder_sender,
        event_loop_waker,
        default_preferences,
        WebDriverIdentity::servo(),
    );
}

/// Starts the WebDriver HTTP server with an embedder-specific identity.
pub fn start_server_with_identity(
    port: u16,
    embedder_sender: Sender<WebDriverCommandMsg>,
    event_loop_waker: Box<dyn EventLoopWaker>,
    default_preferences: Preferences,
    identity: WebDriverIdentity,
) {
    start_server_on_with_identity(
        SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), port),
        embedder_sender,
...
/// Starts the WebDriver HTTP server on an explicit address with an
/// embedder-specific identity.
pub fn start_server_on_with_identity(
    address: SocketAddr,
    embedder_sender: Sender<WebDriverCommandMsg>,
    event_loop_waker: Box<dyn EventLoopWaker>,
    default_preferences: Preferences,
    ...
    thread::Builder::new()
        ...
        .spawn(
            move || match server::start(address, vec![], vec![], handler, extension_routes()) {
...
struct Handler {
    ...
    /// The identity and namespaced capabilities returned to WebDriver clients.
    webdriver_identity: WebDriverIdentity,
}
...
impl Handler {
    ...
    fn handle_new_session(
        ...
        let mut servo_capabilities = ServoCapabilities::with_identity(&self.webdriver_identity);
