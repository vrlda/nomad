...
use servo::{
    ...
    ServoBuilder, SimpleDialog, UserScriptWorld, WebView, WebViewDelegate,
};
...
// Used by isolated-world API tests only.
#[allow(dead_code)]
pub(crate) fn evaluate_javascript_in_world(
    servo_test: &ServoTest,
    webview: WebView,
    script: impl ToString,
    ...
    let saved_result = Rc::new(RefCell::new(None));
    let callback_result = saved_result.clone();
    webview.evaluate_javascript_in_world(script, world, move |result| {
        *callback_result.borrow_mut() = Some(result)
    });
