...
use servo::{
    ...
    UserScript, UserScriptWorld, WebView, WebViewBuilder, WebViewDelegate,
};
use url::Url;
...
#[test]
fn test_user_content_manager_isolated_world_has_private_global() {
    let servo_test = ServoTest::new();
    let user_content_manager = Rc::new(UserContentManager::new(servo_test.servo()));
    user_content_manager.add_script(Rc::new(UserScript::new_in_world(
        "globalThis.__isolatedMarker = 42; document.documentElement.setAttribute('data-nomad-isolated', 'yes'); globalThis.__pageMarker = 7;".into(),
        None,
        UserScriptWorld::Isolated("test-extension".into()),
    )));

    let webview = WebViewBuilder::new(servo_test.servo(), servo_test.rendering_context.clone())
        .user_content_manager(user_content_manager)
        .url(Url::parse("data:text/html,<!DOCTYPE html><title>isolated</title>").unwrap())
        .build();

    let result = evaluate_javascript(
        &servo_test,
        webview,
        "[typeof window.__isolatedMarker, document.documentElement.getAttribute('data-nomad-isolated'), typeof window.__pageMarker]",
    );
    assert_eq!(
        result,
