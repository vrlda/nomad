...
use crate::network_manager::{DownloadCallback, DownloadHandle, NetworkManager};
...
impl ServoInner {
    ...
    fn handle_net_embedder_message(&self, message: NetToEmbedderMsg) {
        match message {
            ...
            NetToEmbedderMsg::WebResourceLoadFinished(
                webview_id,
                method,
                url,
                status,
                ...
                if let Some(webview) =
                    webview_id.and_then(|webview_id| self.get_webview_handle(webview_id))
                {
                    webview.delegate().web_resource_load_finished(
                        webview,
                        method,
                        url,
                        status,
    ...
    fn handle_embedder_message(&self, message: EmbedderMsg) {
        match message {
            ...
            EmbedderMsg::RequestDownload(webview_id, download_request) => {
                if let Some(webview) = self.get_webview_handle(webview_id) {
                    webview
                        .delegate()
                        .request_download(webview, download_request);
                }
            },
        ...
        match message {
            ...
            ConstellationToEmbedderMsg::WebViewClosed(webview_id) => {
                if let Some(webview) = self.get_webview_handle(webview_id) {
                    webview
                        .delegate()
                        .notify_frame_tree_changed(webview.clone(), vec![]);
                    webview.delegate().notify_closed(webview);
                }
            },
            ConstellationToEmbedderMsg::FrameTreeChanged(webview_id, frames) => {
                if let Some(webview) = self.get_webview_handle(webview_id) {
                    webview
                        .delegate()
                        .notify_frame_tree_changed(webview, frames);
                }
            },
...
impl Servo {
    ...
    /// Create a memory report for one WebView.
    pub fn create_webview_memory_report(
        &self,
        webview_id: WebViewId,
        snd: GenericCallback<MemoryReportResult>,
    ) {
        self.0
            .constellation_proxy
            .send(EmbedderToConstellationMessage::CreateWebViewMemoryReport(
                webview_id, snd,
            ));
    }
    ...
    /// Starts a download using Servo's shared network and cookie stack.
    pub fn start_download(
        ...
        self.0
            .network_manager
            .borrow()
            .start_download(webview_id, url, callback)
    }

    /// Cancels a download using its shared network-stack handle.
    pub fn cancel_download(&self, handle: DownloadHandle) {
        self.0.network_manager.borrow().cancel_download(handle);
    }
