...
use embedder_traits::{
    ...
    ContextMenuItem, Cursor, DownloadRequest, EmbedderControlId, EmbedderControlResponse,
    ...
    SimpleDialogRequest, TraversalId, WebResourceRequest, WebResourceRequestDecision,
...
impl WebResourceLoad {
    ...
    /// Continue the request after applying a synchronous blocking decision.
    pub fn continue_request(mut self, decision: WebResourceRequestDecision) {
        if let Err(error) = self
            .responder
            .send(WebResourceResponseMsg::Continue(decision))
        {
            self.error_sender.raise_response_send_error(error);
        }
    }
...
pub trait WebViewDelegate {
    ...
    /// The active frame tree changed. Pipeline handles are opaque to embedder
    /// clients and valid only while present in this snapshot.
    fn notify_frame_tree_changed(
        &self,
        _webview: WebView,
        _frames: Vec<(u64, Option<u64>, String)>,
    ) {
    }
    ...
    /// Request that the embedder start handling a page-triggered download.
    fn request_download(&self, _webview: WebView, _download_request: DownloadRequest) {}
    ...
    /// through [`WebViewDelegate::load_web_resource`] has completed. The
