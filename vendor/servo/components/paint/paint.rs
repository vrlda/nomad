...
impl Paint {
    ...
    fn handle_browser_message(&self, msg: PaintMessage) {
        ...
        match msg {
            ...
            PaintMessage::CollectWebViewMemoryReport(webview_id, sender) => {
                self.collect_webview_memory_report(webview_id, sender);
            },
    ...
    fn collect_webview_memory_report(
        &self,
        webview_id: WebViewId,
        sender: profile_traits::mem::ReportsChan,
    ) {
        let painter_id = webview_id.into();
        let Some(painter) = self.maybe_painter(painter_id) else {
            sender.send(ProcessReports::new(Vec::new()));
            return;
        };
        let memory_report = painter.report_memory();
        let prefix = format!("webview({webview_id:?})");
        let mut reports = vec![
            Report {
                path: path![prefix.clone(), "webrender", "fonts"],
                kind: ReportKind::ExplicitJemallocHeapSize,
                size: memory_report.fonts,
            },
            Report {
                path: path![prefix.clone(), "webrender", "images"],
                kind: ReportKind::ExplicitJemallocHeapSize,
                size: memory_report.images,
            },
            Report {
                path: path![prefix.clone(), "webrender", "display-list"],
                kind: ReportKind::ExplicitJemallocHeapSize,
                size: memory_report.display_list,
            },
        ];
        ...
        perform_memory_report(|ops| {
            let scroll_trees_memory_usage = painter
                .webview_renderer(webview_id)
                .map_or(0, |renderer| renderer.scroll_trees_memory_usage(ops));
            reports.push(Report {
                path: path![prefix, "paint", "scroll-tree"],
                kind: ReportKind::ExplicitJemallocHeapSize,
                size: scroll_trees_memory_usage,
            });
        });
