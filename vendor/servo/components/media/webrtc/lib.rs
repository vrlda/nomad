...
pub type WebRtcDescriptionCallback = Box<dyn FnOnce(WebRtcResult) + Send + 'static>;
...
pub trait WebRtcControllerBackend: Send {
    ...
    fn replace_stream(&mut self, _: &MediaStreamId, _: &MediaStreamId) -> WebRtcResult {
        Err(WebRtcError::Backend(
            "sender track replacement is unavailable in this media backend".to_owned(),
        ))
    }
    fn set_sender_parameters(
        &mut self,
        _: &MediaStreamId,
        _: &RtpSenderParameters,
    ) -> WebRtcResult {
        Err(WebRtcError::Backend(
            "sender parameter reconfiguration is unavailable in this media backend".to_owned(),
        ))
    }
