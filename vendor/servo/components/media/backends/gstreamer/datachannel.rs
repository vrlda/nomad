...
impl GStreamerWebRtcDataChannel {
    ...
    pub fn send(&self, message: &DataChannelMessage) -> Result<(), WebRtcError> {
        if self.channel.ready_state() != WebRTCDataChannelState::Open {
            return Err(WebRtcError::Backend(
                "WebRTC data channel is not open".to_owned(),
            ));
        }
    ...
    pub(crate) fn ready_state(&self) -> WebRTCDataChannelState {
