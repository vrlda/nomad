...
use servo_media_webrtc::{RtpSenderParameters, WebRtcError, WebRtcResult};
...
pub struct GStreamerMediaStream {
    ...
    sender_encoder: Option<gstreamer::Element>,
    pending_sender_parameters: Option<RtpSenderParameters>,
}
...
impl GStreamerMediaStream {
    pub fn new(type_: MediaStreamType, elements: Vec<gstreamer::Element>) -> Self {
        Self {
            ...
            pipeline: None,
            sender_encoder: None,
            pending_sender_parameters: None,
        }
    }
    ...
    pub fn set_sender_parameters(&mut self, parameters: RtpSenderParameters) -> WebRtcResult {
        let [encoding] = parameters.encodings.as_slice() else {
            return Err(WebRtcError::Backend(
                "GStreamer currently supports exactly one RTP encoding per sender".to_owned(),
            ));
        };
        if encoding
            .scale_resolution_down_by
            .is_some_and(|scale| !scale.is_finite() || scale < 1.0)
        {
            return Err(WebRtcError::Backend(
                "scaleResolutionDownBy must be finite and at least 1".to_owned(),
            ));
        }
        if encoding
            .scale_resolution_down_by
        ...
        {
            return Err(WebRtcError::Backend(
                "video scaling is not available in the GStreamer sender pipeline".to_owned(),
            ));
        }

        self.pending_sender_parameters = Some(parameters);
        self.apply_pending_sender_parameters();
        Ok(())
    }

    fn apply_pending_sender_parameters(&mut self) {
        let Some(parameters) = self.pending_sender_parameters.as_ref() else {
            return;
        };
        let Some(encoding) = parameters.encodings.first() else {
            return;
        };
        if let Some(encoder) = &self.sender_encoder
            && let Some(max_bitrate) = encoding.max_bitrate
        {
            match self.type_ {
                MediaStreamType::Video => encoder.set_property("target-bitrate", max_bitrate),
                MediaStreamType::Audio => encoder.set_property("bitrate", max_bitrate),
            }
        }
        self.set_enabled(encoding.active);
    }
    ...
    pub fn encoded(&mut self) -> Result<gstreamer::Element, BoolError> {
        ...
        match self.type_ {
            MediaStreamType::Video => {
                ...
                self.sender_encoder = Some(vp8enc.clone());
            ...
            MediaStreamType::Audio => {
                ...
                self.sender_encoder = Some(opusenc.clone());
