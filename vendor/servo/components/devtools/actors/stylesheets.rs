...
fn stylesheet_index(resource_id: &str) -> i32 {
    resource_id
        .split('-')
        .next_back()
        .unwrap_or("0")
    ...
    fn handle_message(
        ...
        match msg_type {
            ...
            "getText" => {
                ...
                let index = stylesheet_index(resource_id);
            ...
            "setStyleSheetText" => {
                ...
                self.script_sender
                    .send(DevtoolScriptControlMsg::SetStyleSheetText(
                        browsing_context_actor.pipeline_id(),
                        stylesheet_index(resource_id),
                        text.to_owned(),
                        tx,
                    ))
                    .map_err(|_| ActorError::Internal)?;
                let updated = rx.recv().map_err(|_| ActorError::Internal)?;
                request.reply_final(&SetTextReply {
                    from: self.name().into(),
                    updated,
                })?
            },
...
#[cfg(test)]
mod tests {
    use super::stylesheet_index;

    #[test]
    fn stylesheet_index_reads_browsing_context_resource_ids() {
        assert_eq!(stylesheet_index("42-3"), 3);
        assert_eq!(stylesheet_index("invalid"), 0);
    }
}
