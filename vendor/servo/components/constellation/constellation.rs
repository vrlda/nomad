...
use profile_traits::mem::{
    MemoryReport, MemoryReportResult, ProcessReports, ProfilerMsg, Report, ReportsChan,
};
...
use servo_base::id::{
    ...
    ScriptEventLoopId, ServiceWorkerId, WebViewId,
};
...
use servo_constellation_traits::{
    ...
    ServiceWorkerAlgorithm, ServiceWorkerClientFrameType, ServiceWorkerClientInfo,
...
pub struct Constellation<STF, SWF> {
    ...
    /// The active service-worker controller recorded by each live document.
    controlled_clients: FxHashMap<PipelineId, (ServoUrl, ServiceWorkerId, Option<String>)>,
    ...
    SWF: ServiceWorkerManagerFactory,
{
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let mut constellation: Constellation<STF, SWF> = Constellation {
                    ...
                    controlled_clients: Default::default(),
    ...
    /// Return the partition key used by a pipeline's top-level browsing
    /// context. Service-worker client operations must use this value when
    /// selecting or addressing documents; origin and registration scope alone
    /// are not sufficient once third-party storage is partitioned.
    fn top_level_partition_key_for_pipeline(&self, pipeline_id: PipelineId) -> Option<String> {
        let pipeline = self.pipelines.get(&pipeline_id)?;
        pipeline
            .load_data
            .storage_partition_key
            .clone()
            .or_else(|| Some(pipeline.url.origin().ascii_serialization()))
    }

    fn assign_storage_partition_key(
        &self,
        parent_pipeline_id: Option<PipelineId>,
        load_data: &mut LoadData,
    ) {
        let (parent_key, parent_credentialless) = parent_pipeline_id
            .and_then(|pipeline_id| self.pipelines.get(&pipeline_id))
            .map(|pipeline| {
                (
                    pipeline.load_data.storage_partition_key.clone(),
                    pipeline.load_data.credentialless,
                )
            })
            .unwrap_or_default();

        load_data.credentialless |= parent_credentialless;

        if parent_pipeline_id.is_none() && load_data.storage_partition_key.is_some() {
            return;
        }

        let is_inherited_url =
            load_data.url.matches_about_blank() || load_data.url.as_str() == "about:srcdoc";
        load_data.storage_partition_key = Some(if is_inherited_url {
            parent_key.unwrap_or_else(|| load_data.url.origin().ascii_serialization())
        } else {
            child_storage_partition_key(
                parent_key.as_deref(),
                &load_data.url.origin().ascii_serialization(),
                load_data.credentialless,
            )
        });
    }
    ...
    fn new_pipeline(
        ...
        self.assign_storage_partition_key(parent_pipeline_id, &mut load_data);
    ...
    fn handle_request_from_embedder(&mut self, message: EmbedderToConstellationMessage) {
        ...
        match message {
            ...
            EmbedderToConstellationMessage::EvaluateJavaScript(
                ...
                self.handle_evaluate_javascript(webview_id, evaluation_id, script, world);
            },
            EmbedderToConstellationMessage::EvaluateJavaScriptInPipeline(
                webview_id,
                pipeline_id,
                evaluation_id,
                script,
                ...
                self.handle_evaluate_javascript_in_pipeline(
                    webview_id,
                    pipeline_id,
                    evaluation_id,
                    script,
            ...
            EmbedderToConstellationMessage::CreateWebViewMemoryReport(webview_id, sender) => {
                self.create_webview_memory_report(webview_id, sender);
            },
            ...
            EmbedderToConstellationMessage::AccessibilityAction(webview_id, request) => {
                self.handle_accessibility_action(webview_id, request);
            },
        }
    }
    ...
    fn handle_evaluate_javascript(
        ...
        script: String,
        world: embedder_traits::user_contents::UserScriptWorld,
    ) {
    ...
    #[servo_tracing::instrument(skip_all)]
    fn handle_evaluate_javascript_in_pipeline(
        &mut self,
        webview_id: WebViewId,
        pipeline_id: PipelineId,
        evaluation_id: JavaScriptEvaluationId,
    ...
    fn handle_request_from_script(
        ...
        match content {
            ...
            ScriptToConstellationMessage::GetBrowsingContextInfo(pipeline_id, response_sender) => {
                let result = self.pipelines.get(&pipeline_id).and_then(|pipeline| {
                    self.browsing_contexts
                        .get(&pipeline.browsing_context_id)
                        .map(|ctx| (ctx.id, ctx.parent_pipeline_id, pipeline.opener))
                });
            ...
            ScriptToConstellationMessage::GetTopLevelUrlForBrowsingContext(
                browsing_context_id,
                response_sender,
            ) => {
                let result = self
                    ...
                    .and_then(|bc| {
                        self.webviews
                            .get(&bc.webview_id)
                            .and_then(|webview| webview.active_top_level_pipeline_id)
                            .and_then(|pipeline_id| self.pipelines.get(&pipeline_id))
                            .map(|pipeline| pipeline.url.clone())
                    });
                if let Err(e) = response_sender.send(result) {
                    warn!("Sending reply to get top-level URL failed ({:?}).", e);
                }
            },
            ...
            ScriptToConstellationMessage::ServiceWorkerClientQuery {
                origin,
                scope_url,
                worker_id,
                partition_key,
                ...
                if let Some(source_pipeline) = self.pipelines.get(&source_pipeline_id)
                    && source_pipeline.url.origin() != origin
                {
                    let _ = result_handler.send(Err(
                        "Service-worker client query crossed an origin boundary".to_owned(),
                    ));
                    return;
                }
                if self
                    .top_level_partition_key_for_pipeline(source_pipeline_id)
                    .as_deref()
                    != partition_key.as_deref()
                {
                    let _ = result_handler.send(Err(
                        "Service-worker client query crossed a storage partition boundary"
                            .to_owned(),
                    ));
                    return;
                }
                let clients = self
                    ...
                    .filter(|pipeline| {
                        ...
                        if self
                            .top_level_partition_key_for_pipeline(pipeline.id)
                            .as_deref()
                            != partition_key.as_deref()
                        {
                            return false;
                        ...
                        include_uncontrolled
                            || self.controlled_clients.get(&pipeline.id).is_some_and(
                                |(controlled_scope, controlled_worker, controlled_partition)| {
                                    controlled_scope == &scope_url
                                        && controlled_worker == &worker_id
                                        && controlled_partition.as_deref()
                                            == partition_key.as_deref()
                                },
                            )
                    })
                    .map(|pipeline| {
                        let frame_type = self
                            ...
                            .map_or(ServiceWorkerClientFrameType::TopLevel, |_| {
                    ...
                    .collect();
                let _ = result_handler.send(Ok(clients));
            },
            ScriptToConstellationMessage::ServiceWorkerClientFocus {
                target_pipeline_id,
                scope_url,
                origin,
                worker_id: _,
                ...
                let Some(target_pipeline) = self.pipelines.get(&target_pipeline_id) else {
                    return warn!("Service-worker focus target is a closed pipeline");
                };
                if self
                    .top_level_partition_key_for_pipeline(source_pipeline_id)
                    .as_deref()
                ...
                {
                    return warn!(
                        "Service-worker focus request came from the wrong storage partition"
                    );
                }
                if scope_url.origin() != origin
                    || target_pipeline.url.origin() != origin
                    || !target_pipeline.url.path().starts_with(scope_url.path())
                    || self
                        .top_level_partition_key_for_pipeline(target_pipeline_id)
                        .as_deref()
                        != partition_key.as_deref()
                {
                    return warn!(
                        "Service-worker focus request crossed an origin or partition boundary"
                    );
                }
                self.handle_focus_remote_browsing_context(
                    target_pipeline.browsing_context_id,
                    RemoteFocusOperation::Viewport,
                );
            },
            ScriptToConstellationMessage::ServiceWorkerClientNavigate {
                target_pipeline_id,
                url,
                scope_url,
                origin,
                ...
                let Some(target_pipeline) = self.pipelines.get(&target_pipeline_id) else {
                    return warn!("Service-worker navigation target is a closed pipeline");
                };
                if self
                    .top_level_partition_key_for_pipeline(source_pipeline_id)
                    .as_deref()
                ...
                {
                    return warn!(
                        "Service-worker navigation request came from the wrong storage partition"
                    );
                }
                if scope_url.origin() != origin
                    || url.origin() != origin
                    || !target_pipeline.url.origin().eq(&origin)
                ...
                {
                    return warn!(
                        "Service-worker navigation rejected an unsupported origin, partition, or URL scheme"
                    );
                }
                self.schedule_navigation(
                    target_pipeline.webview_id,
                    target_pipeline.id,
            ...
            ScriptToConstellationMessage::ServiceWorkerClientPostMessage {
                ...
                let Some(target_pipeline) = self.pipelines.get(&target_pipeline_id) else {
                    return warn!("Service-worker message target is a closed pipeline");
                };
                if self
                    .top_level_partition_key_for_pipeline(source_pipeline_id)
                    .as_deref()
                    != partition_key.as_deref()
                {
                    return warn!("Service-worker message came from the wrong storage partition");
                }
                if target_pipeline.url.origin() != origin
                    || scope_url.origin() != origin
                    || self
                        .top_level_partition_key_for_pipeline(target_pipeline_id)
                        .as_deref()
                        != partition_key.as_deref()
                {
                    return warn!(
                        "Service-worker message target is outside the registration origin or partition"
                    );
                }
                if let Some(source_pipeline) = self.pipelines.get(&source_pipeline_id)
                    && source_pipeline.url.origin() != origin
                {
                    return warn!("Service-worker message crossed an origin boundary");
                }
                if let Err(error) =
                    target_pipeline
                        .event_loop
                        .send(ScriptThreadMessage::ServiceWorkerPostMessage {
                ...
                {
                    self.handle_send_error(target_pipeline_id, error);
                }
            },
            ScriptToConstellationMessage::ServiceWorkerClaimClients {
                origin,
                scope_url,
                script_url,
                worker_id,
                ...
                {
                    let _ = result_handler.send(Err(
                        "Service-worker claim crossed an origin boundary".to_owned(),
                    ));
                    return;
                }
                if self
                ...
                {
                    let _ = result_handler.send(Err(
                        "Service-worker claim crossed a storage partition boundary".to_owned(),
                    ));
                    return;
                }
                if scope_url.origin() != origin || script_url.origin() != origin {
                    let _ = result_handler.send(Err(
                        "Service-worker claim crossed an origin boundary".to_owned(),
                    ));
                    return;
                ...
                let targets = self
                    ...
                    .values()
                    .filter(|pipeline| {
                        pipeline.url.origin() == origin
                            && pipeline.url.path().starts_with(scope_url.path())
                            && self
                                .top_level_partition_key_for_pipeline(pipeline.id)
                                .as_deref()
                                == partition_key.as_deref()
                    ...
                    .collect::<Vec<_>>();
                for target_pipeline_id in targets {
                    self.controlled_clients.insert(
                        target_pipeline_id,
                        (scope_url.clone(), worker_id, partition_key.clone()),
                    );
                    let Some(target_pipeline) = self.pipelines.get(&target_pipeline_id) else {
                        continue;
                    };
                    if let Err(error) = target_pipeline.event_loop.send(
                        ScriptThreadMessage::ServiceWorkerControllerUpdate {
                            pipeline_id: target_pipeline_id,
            ...
            ScriptToConstellationMessage::ServiceWorkerControllerChanged {
                ...
                let Some(source_pipeline) = self.pipelines.get(&source_pipeline_id) else {
                    return warn!("Service-worker controller update came from a closed pipeline");
                };
                if source_pipeline.url.origin() != scope_url.origin() {
                    return warn!("Service-worker controller update crossed an origin boundary");
                }
                if self
                    .top_level_partition_key_for_pipeline(source_pipeline_id)
                    .as_deref()
                ...
                {
                    return warn!(
                        "Service-worker controller update crossed a storage partition boundary"
                    );
                }
                if let Some(mgr) = self.sw_managers.get(&scope_url.origin()) {
                    let _ = mgr.send(ServiceWorkerMsg::ControllerChanged {
                ...
                match worker_id {
                    Some(worker_id) => {
                        self.controlled_clients
                            .insert(source_pipeline_id, (scope_url, worker_id, partition_key));
                    },
                    None => {
                        if self
                            .controlled_clients
                            .get(&source_pipeline_id)
                            .is_some_and(|(controlled_scope, _, controlled_partition)| {
                                controlled_scope == &scope_url
                                    && controlled_partition.as_deref() == partition_key.as_deref()
                            })
                        {
                            self.controlled_clients.remove(&source_pipeline_id);
                        }
                    },
                }
            },
            ScriptToConstellationMessage::ServiceWorkerOpenWindow {
                origin,
                scope_url,
                script_url: _,
                worker_id: _,
                ...
                {
                    let _ = result_handler.send(Err(
                        "Service-worker openWindow crossed an origin boundary".to_owned(),
                    ));
                    return;
                }
                if scope_url.origin() != origin || url.origin() != origin {
                    let _ = result_handler.send(Err(
                        "Service-worker openWindow URL is not same-origin".to_owned(),
                    ));
                    return;
                }
                if !self.webviews.contains_key(&opener_webview_id) {
                    let _ = result_handler.send(Ok(None));
                    return;
                }
                let Some((webview_id_sender, webview_id_receiver)) = generic_channel::channel()
                else {
                    let _ = result_handler.send(Err(
                        "Failed to create a service-worker window request".to_owned(),
                    ));
                    return;
                };
                self.constellation_to_embedder_proxy.send(
                ...
                let details = match webview_id_receiver.recv() {
                ...
                let pipeline_id = self.handle_new_top_level_browsing_context(url.clone(), details);
                let _ = result_handler.send(Ok(Some(ServiceWorkerClientInfo {
                    pipeline_id,
                    url,
                    frame_type: ServiceWorkerClientFrameType::TopLevel,
                })));
            },
            ScriptToConstellationMessage::ForwardDOMMessage(msg_vec, scope_url, partition_key) => {
                if let Some(mgr) = self.sw_managers.get(&scope_url.origin()) {
                    let _ = mgr.send(ServiceWorkerMsg::ForwardDOMMessage(
                        msg_vec,
                        scope_url,
                        partition_key,
                    ));
    ...
    fn handle_serviceworker_algorithm(
        ...
        let origin = match &algorithm {
            ...
            ServiceWorkerAlgorithm::Update(job) => job.storage_key.clone(),
            ServiceWorkerAlgorithm::Activate { storage_key, .. } => storage_key.clone(),
            ...
            ServiceWorkerAlgorithm::GetRegistrations { storage_key, .. } => storage_key.clone(),
            ServiceWorkerAlgorithm::SetNavigationPreload { storage_key, .. } => storage_key.clone(),
        };
    ...
    fn handle_accessibility_action(
        ...
        self.send_message_to_pipeline(
            ...
            "Accessibility action targeted a closed pipeline",
        );
    }
    ...
    fn create_webview_memory_report(
        &self,
        webview_id: WebViewId,
        callback: GenericCallback<MemoryReportResult>,
    ) {
        let mut event_loop_groups: Vec<(Rc<EventLoop>, Vec<PipelineId>)> = Vec::new();
        for pipeline in self
            .pipelines
            .values()
            .filter(|pipeline| pipeline.webview_id == webview_id)
        {
            if let Some((_, pipeline_ids)) = event_loop_groups
                .iter_mut()
                .find(|(event_loop, _)| Rc::ptr_eq(event_loop, &pipeline.event_loop))
            {
                pipeline_ids.push(pipeline.id);
            } else {
                event_loop_groups.push((pipeline.event_loop.clone(), vec![pipeline.id]));
            }
        }

        let (report_sender, report_receiver) =
            generic_channel::channel().expect("Failed to create WebView memory report channel");
        let report_sender = ReportsChan(report_sender);
        self.paint_proxy
            .send(PaintMessage::CollectWebViewMemoryReport(
                webview_id,
                report_sender.clone(),
            ));

        let mut expected_reports = 1;
        for (event_loop, pipeline_ids) in event_loop_groups {
            if event_loop
                .send(ScriptThreadMessage::CollectWebViewMemoryReport(
                    webview_id,
                    pipeline_ids,
                    report_sender.clone(),
                ))
        ...
        thread::spawn(move || {
            let mut reports_by_process: HashMap<u32, Vec<Report>> = HashMap::new();
            for _ in 0..expected_reports {
                let Ok(ProcessReports { pid, reports }) = report_receiver.recv() else {
                    break;
                };
                reports_by_process.entry(pid).or_default().extend(reports);
            }

            let results = reports_by_process
                .into_iter()
                .map(|(pid, reports)| MemoryReport {
                    pid,
                    is_main_process: pid == process::id(),
                    reports,
                })
                .collect();
            let _ = callback.send(MemoryReportResult { results });
        });
    }
    ...
    fn handle_script_new_iframe(&mut self, load_info: IFrameLoadInfoWithData) {
        ...
        let is_private = is_private || is_parent_private;
        let mut load_data = load_info.load_data;
        self.assign_storage_partition_key(Some(parent_pipeline_id), &mut load_data);
    ...
    fn handle_traverse_history_msg(
        ...
        // A nested document restored from an inactive history entry must run
        // its navigation fetch again. This preserves traversal request flags
        // and gives the containing iframe a completion notification.
        let mut child_reload_targets = Vec::new();
        for (browsing_context_id, reloader) in browsing_context_changes.iter_mut() {
            let NeedsToReload::No(pipeline_id) = *reloader else {
                continue;
            };
            let is_child_context = self
                .browsing_contexts
                .get(browsing_context_id)
                .is_some_and(|context| context.parent_pipeline_id.is_some());
            if !is_child_context {
                continue;
            }
            ...
            *reloader = NeedsToReload::Yes(pipeline_id, load_data.clone());
            child_reload_targets.push((pipeline_id, load_data));
        }
        if let Some(webview) = self.webviews.get_mut(&webview_id) {
            for (pipeline_id, load_data) in child_reload_targets {
    ...
    fn handle_activate_document_msg(&mut self, pipeline_id: PipelineId) {
        ...
        let update_reason = if matches!(&change.replace, Some(NeedsToReload::Yes(..))) {
            UpdatePipelineIdReason::Traversal
        } else {
    ...
    fn flatten_frame_snapshot(
        &self,
        tree: &SendableFrameTree,
        parent: Option<PipelineId>,
        output: &mut Vec<(u64, Option<u64>, String)>,
    ) {
        output.push((
            u64::from(tree.pipeline.id),
            parent.map(u64::from),
            self.pipelines
                .get(&tree.pipeline.id)
                .map_or_else(String::new, |pipeline| pipeline.url.as_str().to_owned()),
        ));
        for child in &tree.children {
            self.flatten_frame_snapshot(child, Some(tree.pipeline.id), output);
        }
    }
    ...
    fn set_frame_tree_for_webview(&mut self, webview_id: WebViewId) {
        ...
        self.flatten_frame_snapshot(&frame_tree, None, &mut snapshot);
        self.constellation_to_embedder_proxy
            .send(ConstellationToEmbedderMsg::FrameTreeChanged(
                webview_id, snapshot,
            ));
