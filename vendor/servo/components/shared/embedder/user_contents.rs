...
/// The JavaScript global in which an embedder user script executes.
///
/// `Page` is the document's normal global. `Isolated` creates a distinct
/// SpiderMonkey global for the supplied stable world name while still
/// allowing Servo to provide controlled wrappers for the document it is
/// attached to.
#[derive(Clone, Debug, Deserialize, Eq, Hash, MallocSizeOf, PartialEq, Serialize)]
pub enum UserScriptWorld {
    Page,
    Isolated(String),
}

impl Default for UserScriptWorld {
    fn default() -> Self {
        Self::Page
    }
}
...
impl UserScript {
    ...
    pub fn new(script: String, source_file: Option<PathBuf>) -> Self {
        Self {
            ...
            script,
            source_file,
            world: UserScriptWorld::Page,
        }
    }

    /// Create a user script that executes in a named engine-managed world.
    pub fn new_in_world(
        script: String,
        source_file: Option<PathBuf>,
        world: UserScriptWorld,
    ...
    /// Returns the execution world for this script.
    pub fn world(&self) -> &UserScriptWorld {
        &self.world
    }
}
