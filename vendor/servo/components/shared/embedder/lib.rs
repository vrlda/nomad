...
/// A request from page content to download a hyperlink instead of navigating to it.
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct DownloadRequest {
    /// The resolved resource URL.
    pub url: Url,
    /// The filename requested by the page, or a browser-generated fallback.
...
pub enum EmbedderMsg {
    ...
    /// Ask the embedder to handle a page-triggered download.
    RequestDownload(WebViewId, DownloadRequest),
...
pub struct WebResourceRequest {
    ...
    /// Known upload size; body bytes are never retained for streaming uploads.
    pub request_body_size: Option<usize>,
    pub request_body_unavailable: bool,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct WebResourceRequestDecision {
    /// Cancel before network I/O starts.
    pub cancel: bool,
    /// Replace request URL before network I/O starts.
    pub redirect_url: Option<Url>,
    /// Case-insensitive request header replacements. Empty values remove headers.
    #[serde(
        deserialize_with = "::hyper_serde::deserialize",
        serialize_with = "::hyper_serde::serialize"
    )]
    pub request_headers: HeaderMap,
    /// Optional response header replacements applied after headers arrive.
    #[serde(
        deserialize_with = "::hyper_serde::deserialize",
        serialize_with = "::hyper_serde::serialize"
    )]
...
pub enum WebResourceResponseMsg {
    /// Continue this request with the supplied blocking decision.
    Continue(WebResourceRequestDecision),
