...
type RenderToParentCallback = Box<dyn Fn(&glow::Context, Rect<i32>, i32) + Send + Sync>;
...
impl OffscreenRenderingContext {
    ...
    pub fn render_to_parent_callback(&self) -> Option<RenderToParentCallback> {
        ...
        let size = Size2D::new(size.width as i32, size.height as i32);
        Some(Box::new(move |gl, target_rect, corner_radius| {
            Self::blit_rounded_framebuffer(
                gl,
    ...
    fn blit_rounded_framebuffer(
        ...
        let radius = corner_radius
            .max(0)
            .min(target_rect.width() / 2)
            .min(target_rect.height() / 2);
        if radius == 0 {
        ...
        let center_source_bottom = radius * source_rect.height() / target_rect.height();
        let center_source_top =
            (target_rect.height() - radius) * source_rect.height() / target_rect.height();
        Self::blit_framebuffer(
            gl,
            Rect::new(
                Point2D::new(
                    source_rect.origin.x,
                    source_rect.origin.y + center_source_bottom,
                ),
                Size2D::new(
                    source_rect.width(),
                    center_source_top - center_source_bottom,
                ),
            ),
            source_framebuffer_id,
            Rect::new(
                Point2D::new(target_rect.origin.x, target_rect.origin.y + radius),
                Size2D::new(target_rect.width(), target_rect.height() - radius * 2),
            ),
            target_framebuffer_id,
        );

        for y in (0..radius).chain(target_rect.height() - radius..target_rect.height()) {
            let edge_distance = y.min(target_rect.height() - 1 - y);
            let inset = if edge_distance >= radius {
                0
            } else {
                let dy = f64::from(radius - edge_distance) - 0.5;
                (f64::from(radius) - (f64::from(radius * radius) - dy * dy).sqrt()).ceil() as i32
            };
            let target_width = target_rect.width() - inset * 2;
            if target_width <= 0 {
                continue;
            }
            let source_left = inset * source_rect.width() / target_rect.width();
            let source_right =
                (target_rect.width() - inset) * source_rect.width() / target_rect.width();
            let source_bottom = y * source_rect.height() / target_rect.height();
            let source_top = (y + 1) * source_rect.height() / target_rect.height();
            Self::blit_framebuffer(
                gl,
                Rect::new(
                    Point2D::new(
                        source_rect.origin.x + source_left,
                        source_rect.origin.y + source_bottom,
                    ),
                    Size2D::new(source_right - source_left, source_top - source_bottom),
                ),
                source_framebuffer_id,
                Rect::new(
                    Point2D::new(target_rect.origin.x + inset, target_rect.origin.y + y),
                    Size2D::new(target_width, 1),
                ),
                target_framebuffer_id,
            );
        }
