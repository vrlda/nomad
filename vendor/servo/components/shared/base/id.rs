...
impl PipelineId {
    /// Reconstruct a pipeline identifier previously obtained from `u64::from`.
    ///
    /// Embedders should treat this value as an opaque handle and only use it
    /// for live pipelines belonging to the same Servo instance.
    pub fn from_u64(value: u64) -> Option<Self> {
        let namespace_id = PipelineNamespaceId((value >> 32) as u32);
        let index = NonZeroU32::new(value as u32)?;
        Some(Self {
            namespace_id,
