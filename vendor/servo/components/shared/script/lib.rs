...
pub enum ScriptThreadMessage {
    ...
    /// Deliver a structured-clone message from a service worker to a window pipeline.
    ServiceWorkerPostMessage {
        ...
        script_url: ServoUrl,
        /// The service-worker instance that sent the message.
    ...
    /// Update a document's ServiceWorkerContainer controller after a claim.
    ServiceWorkerControllerUpdate {
        /// The target pipeline.
        pipeline_id: PipelineId,
        /// The service-worker registration scope.
        scope_url: ServoUrl,
        /// The active service-worker script URL.
        script_url: ServoUrl,
        /// The active service-worker instance.
        worker_id: ServiceWorkerId,
        /// The registration owning the active worker.
    ...
    EvaluateJavaScript(
        ...
        String,
        embedder_traits::user_contents::UserScriptWorld,
    ),
    ...
    /// Perform an assistive-technology action on a node in the given document.
    AccessibilityAction(PipelineId, ActionRequest),
    /// Force a garbage collection in this script thread.
    TriggerGarbageCollection,
    /// Measure the memory owned by the selected WebView pipelines.
    CollectWebViewMemoryReport(WebViewId, Vec<PipelineId>, mem::ReportsChan),
}
