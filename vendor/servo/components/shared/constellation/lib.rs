...
pub enum EmbedderToConstellationMessage {
    ...
    /// Create a memory report for one WebView and return it via the [`GenericCallback`].
    CreateWebViewMemoryReport(WebViewId, GenericCallback<MemoryReportResult>),
    ...
    /// Forward an assistive-technology action to the active document in the given `WebView`.
    AccessibilityAction(WebViewId, ActionRequest),
}
