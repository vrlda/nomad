...
pub struct LoadData {
    ...
    /// The storage partition selected for this document and its workers.
    ///
    /// This is separate from `top_level_creation_url`: that URL is used by
    /// secure-context and environment algorithms, while storage partitioning
    /// also depends on cross-site ancestors and credentialless embedding.
    pub storage_partition_key: Option<String>,

    /// Whether this load was requested by a credentialless iframe.
...
/// Build the storage partition key for a child document.
///
/// The key records the origin chain only when it changes. A credentialless
/// subtree gets one additional stable component, so sibling credentialless
/// frames share storage while remaining isolated from ordinary frames.
pub fn child_storage_partition_key(
    parent_key: Option<&str>,
    origin: &str,
    credentialless: bool,
) -> String {
    const CREDENTIALLESS_MARKER: &str = "credentialless";

    let mut components = parent_key
        .map(|key| key.split('|').map(str::to_owned).collect::<Vec<_>>())
        .unwrap_or_default();

    if credentialless
        && !components
            .iter()
            .any(|component| component == CREDENTIALLESS_MARKER)
    {
        components.push(CREDENTIALLESS_MARKER.to_owned());
    }

    if components
        .last()
        .is_none_or(|component| component != origin)
    {
...
#[cfg(test)]
mod tests {
    use super::child_storage_partition_key;

    #[test]
    fn same_origin_descendants_share_a_partition() {
        assert_eq!(
            child_storage_partition_key(Some("https://a.test"), "https://a.test", false),
            "https://a.test"
        );
    }

    #[test]
    fn cross_site_ancestors_remain_in_the_partition_key() {
        assert_eq!(
    ...
    #[test]
    fn credentialless_siblings_share_a_distinct_partition() {
        assert_eq!(
            child_storage_partition_key(Some("https://a.test"), "https://b.test", true),
            "https://a.test|credentialless|https://b.test"
        );
        assert_eq!(
            child_storage_partition_key(
                Some("https://a.test|credentialless|https://b.test"),
                "https://b.test",
                true,
            ),
            "https://a.test|credentialless|https://b.test"
...
/// Metadata for a browsing-context client returned by the service-worker
/// `clients.matchAll()` algorithm.
#[derive(Clone, Debug, Deserialize, MallocSizeOf, Serialize)]
pub struct ServiceWorkerClientInfo {
    /// The pipeline-backed client identifier exposed by `Client.id`.
    pub pipeline_id: PipelineId,
    /// The client's current URL.
    pub url: ServoUrl,
    /// Whether the client is a top-level document or a nested frame.
    pub frame_type: ServiceWorkerClientFrameType,
}

/// Frame categories needed by the service-worker Client interface.
#[derive(Clone, Copy, Debug, Deserialize, MallocSizeOf, Serialize)]
pub enum ServiceWorkerClientFrameType {
...
pub enum ServiceWorkerMsg {
    /// Update the network manager's view of a document controlled by a
    /// service worker so same-origin subresource requests are intercepted.
    ControllerChanged {
    ...
    ForwardDOMMessage(DOMMessage, ServoUrl, Option<String>),
    ...
    /// Request activation of a waiting worker from its own global scope.
    SkipWaiting {
        /// The registration scope owning the worker.
        scope_url: ServoUrl,
        /// The worker requesting activation.
        worker_id: ServiceWorkerId,
    },
    /// Notification from a worker after its install event and wait-until
    /// promises have completed.
    InstallFinished {
        /// The registration scope owning the worker.
        scope_url: ServoUrl,
        /// The worker that completed installation.
        worker_id: ServiceWorkerId,
        /// Whether installation completed successfully.
        succeeded: bool,
        /// Whether the fetched script response permits this registration scope.
        scope_allowed: bool,
    },
    /// Notification from a worker after its activate event and wait-until
    /// promises have completed.
    ActivateFinished {
        /// The registration scope owning the worker.
        scope_url: ServoUrl,
        /// The worker that completed activation.
        worker_id: ServiceWorkerId,
        /// Whether activation completed before the worker lifetime expired.
        succeeded: bool,
    },
...
pub enum JobResultValue {
    ...
    /// A registration update completed successfully.
    Update {
        /// The registration snapshot exposed while the new worker is
        /// installing and the update promise resolves.
        registration: ServiceWorkerRegistrationInfo,
        /// The lifecycle snapshot to apply in a later task, after updatefound
        /// observers have seen the installing worker.
        state: ServiceWorkerRegistrationInfo,
        /// Whether activation should be requested after the state snapshot is
        /// applied to the client registration object.
...
pub enum ServiceWorkerAlgorithm {
    ...
    StartRegister(Job),
    /// Run the update algorithm for an existing registration.
    Update(Job),
    /// Activate a newly installed worker after its installing state has been
    /// exposed to the client registration object.
    Activate {
    ...
    /// Update navigation-preload state for an existing registration.
...
pub enum ServiceWorkerAlgorithmResult {
    ...
    /// <https://w3c.github.io/ServiceWorker/#get-registrations-algorithm>
    GetRegistrations(Vec<ServiceWorkerRegistrationInfo>),

    /// Notify an existing client registration object that worker lifecycle
    /// state changed outside a pending registration algorithm.
    RegistrationStateChanged(ServiceWorkerRegistrationInfo),
...
pub struct Job {
    ...
    pub storage_key: ImmutableOrigin,
    /// The top-level site partition containing this registration.
    pub partition_key: Option<String>,
    /// Resolve the job as an update rather than as a registration result.
    pub expect_update_result: bool,
}
...
pub struct WorkerGlobalScopeInit {
    ...
    /// The origin
    pub origin: ImmutableOrigin,
    /// The top-level creation URL used to derive storage partitions.
    pub top_level_creation_url: Option<ServoUrl>,
    /// The storage partition key used by this worker and its clients.
    pub storage_partition_key: Option<String>,
...
pub enum ScriptToConstellationMessage {
    ...
    /// Enumerate live same-origin browsing-context clients for a service worker.
    ServiceWorkerClientQuery {
        origin: ImmutableOrigin,
        scope_url: ServoUrl,
        worker_id: ServiceWorkerId,
        /// The top-level site partition of the service-worker global.
        partition_key: Option<String>,
        include_uncontrolled: bool,
        result_handler: GenericCallback<Result<Vec<ServiceWorkerClientInfo>, String>>,
    },
    /// Focus a window client selected by a service worker.
    ...
    /// Navigate a window client selected by a service worker.
    ServiceWorkerClientNavigate {
        target_pipeline_id: PipelineId,
        url: ServoUrl,
        scope_url: ServoUrl,
        origin: ImmutableOrigin,
    ...
    /// Deliver a structured-clone message from a service worker to one client.
    ...
    /// Claim same-origin clients under a service-worker registration scope.
    ServiceWorkerClaimClients {
        origin: ImmutableOrigin,
        scope_url: ServoUrl,
        script_url: ServoUrl,
        worker_id: ServiceWorkerId,
        ...
        /// The top-level site partition containing this registration.
        partition_key: Option<String>,
        result_handler: GenericCallback<Result<(), String>>,
    },
    /// Record a document's current service-worker controller.
    ServiceWorkerControllerChanged {
        scope_url: ServoUrl,
        worker_id: Option<ServiceWorkerId>,
        /// The top-level site partition of the controlled document.
        partition_key: Option<String>,
    },
    /// Open a new top-level browsing context from a service worker.
    ServiceWorkerOpenWindow {
        ...
        url: ServoUrl,
        result_handler: GenericCallback<Result<Option<ServiceWorkerClientInfo>, String>>,
    },
    ...
    /// Get the current URL of the top-level browsing context for a given browsing context.
    GetTopLevelUrlForBrowsingContext(BrowsingContextId, GenericSender<Option<ServoUrl>>),
    /// Get the browsing context id of the browsing context in which pipeline is
    /// embedded, the parent pipeline id of that browsing context, and the
    /// opener browsing context id recorded for the pipeline (used so
    /// dissimilar-origin proxies for top-level contexts keep the real opener
    /// instead of inheriting the querying pipeline's).
