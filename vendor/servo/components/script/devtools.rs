...
use devtools_traits::{
    ...
    DevtoolsStorage, DevtoolsStorageEntry, EventListenerInfo, GetHTMLType, MatchedRule, NodeInfo,
    NodeStyle, RuleModification, StyleSheetInfo, TimelineMarker, TimelineMarkerType,
};
...
pub(crate) fn handle_get_storage(
    cx: &mut JSContext,
    documents: &DocumentCollection,
    pipeline: PipelineId,
    reply: GenericSender<DevtoolsStorage>,
) {
    let storage = (|| {
        let window = documents.find_window(pipeline)?;
        let mut realm = enter_auto_realm(cx, &*window);
        let cx = &mut realm.current_realm();
        let local_storage = window
            .GetLocalStorage(cx)
            .ok()
            .map(|storage| storage_entries(&storage))?;
        let session_storage = window
            .GetSessionStorage(cx)
            .ok()
            .map(|storage| storage_entries(&storage))?;
        let cookies = window
            .Document()
            .GetCookie()
            .ok()
            .map(|cookies| cookie_entries(&cookies.to_string()))?;
        Some(DevtoolsStorage {
            local_storage,
            session_storage,
            cookies,
        })
    ...
    .unwrap_or_default();
    let _ = reply.send(storage);
}

fn storage_entries(storage: &Storage) -> Vec<DevtoolsStorageEntry> {
    (0..storage.Length())
        .filter_map(|index| {
            let key = storage.Key(index)?;
            let value = storage.GetItem(key.clone());
            Some(DevtoolsStorageEntry {
                key: key.into(),
...
fn cookie_entries(cookies: &str) -> Vec<DevtoolsStorageEntry> {
    cookies
        .split(';')
        .filter_map(|cookie| {
...
pub(crate) fn handle_set_stylesheet_text(
    cx: &mut JSContext,
    documents: &DocumentCollection,
    pipeline: PipelineId,
    index: i32,
    text: String,
    reply: GenericSender<bool>,
) {
    let updated = (|| {
        let document = documents.find_document(pipeline)?;
        let stylesheet = document
        ...
        if let Some(owner) = stylesheet.owner_node() {
            owner
                .upcast::<Node>()
                .set_text_content_for_element(cx, Some(DOMString::from(text)));
        } else {
            stylesheet.replace_text_for_devtools(cx.no_gc(), text);
        }
        Some(true)
    })()
    .unwrap_or(false);
    reply.send(updated).unwrap();
}
