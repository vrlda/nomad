...
pub(crate) struct GlobalScope {
    ...
    /// The storage partition key for this environment.
    storage_partition_key: Option<String>,
...
impl GlobalScope {
    ...
    pub(crate) fn get_serviceworker_registration(
        ...
        if let Some(worker_id) = installing_worker {
            ...
            new_registration.set_installing(cx, &worker);
        }

        if let Some(worker_id) = waiting_worker {
            let worker = self.get_serviceworker(cx, script_url, scope, worker_id);
            new_registration.set_waiting(cx, &worker);
        }

        if let Some(worker_id) = active_worker {
            let worker = self.get_serviceworker(cx, script_url, scope, worker_id);
            new_registration.set_active(cx, &worker);
        }
    ...
    pub(crate) fn get_serviceworker_registration_for_scope(
        &self,
        scope: &ServoUrl,
        ...
        self.registration_map
            .borrow()
            .values()
            .find(|registration| registration.scope_url() == scope)
    ...
    pub(crate) fn storage_partition_key(&self) -> Option<&str> {
        self.storage_partition_key.as_deref()
    }
    ...
    pub(crate) fn image_cache(&self) -> Arc<dyn ImageCache> {
        ...
        if let Some(dissimilar) = self.downcast::<DissimilarOriginWindow>() {
            return dissimilar.image_cache();
        }
        unreachable!();
    }
    ...
    pub(crate) fn request_client(&self, no_gc: Option<&NoGC>) -> RequestClient {
        ...
        RequestClient {
            ...
            top_level_partition_key: self.storage_partition_key().map(str::to_owned),
        }
    }
    ...
    pub(crate) fn api_base_url(&self) -> ServoUrl {
        ...
        if let Some(dissimilar) = self.downcast::<DissimilarOriginWindow>() {
            return dissimilar
                ...
                .unwrap_or_else(|| self.creation_url());
        }
    ...
    pub(crate) fn get_referrer_policy(&self) -> ReferrerPolicy {
        ...
        if let Some(dissimilar) = self.downcast::<DissimilarOriginWindow>() {
            return dissimilar
                .document()
                .map(|document| document.get_referrer_policy())
                .unwrap_or_default();
        }
        unreachable!();
    }
    ...
    pub(crate) fn insecure_requests_policy(&self) -> InsecureRequestsPolicy {
        ...
        if let Some(dissimilar) = self.downcast::<DissimilarOriginWindow>() {
            return dissimilar
                .document()
                .map(|document| document.insecure_requests_policy())
    ...
    pub(crate) fn event_loop_sender(&self) -> Option<ScriptEventLoopSender> {
        if let Some(window) = self.downcast::<Window>() {
            ...
            dissimilar.event_loop_sender()
        } else {
            unreachable!(
                "Tried to access event loop sender for incompatible \
                 GlobalScope (PaintWorklet)"
            );
        }
    }
    ...
    pub(crate) fn perform_a_microtask_checkpoint(&self, cx: &mut js::context::JSContext) {
        ...
        } else if self.is::<DissimilarOriginWindow>() {
            self.downcast::<DissimilarOriginWindow>()
                .expect("global type changed during microtask checkpoint")
                .document()
                .expect("isolated user-script world has no document")
                .window()
                .perform_a_microtask_checkpoint(cx);
        }
    }
    ...
    pub(crate) fn new_script_pair(&self) -> (ScriptEventLoopSender, ScriptEventLoopReceiver) {
        ...
        if let Some(dissimilar) = self.downcast::<DissimilarOriginWindow>() {
            return dissimilar.new_script_pair();
        }
        unreachable!();
    }
    ...
    pub(crate) fn is_secure_context(&self) -> bool {
        ...
        match self.top_level_creation_url() {
            ...
            Some(top_level_creation_url)
                if !self.is::<WorkerGlobalScope>() && !self.is::<WorkletGlobalScope>() =>
            {
            ...
            Some(_) => {
                // Worker globals may carry the registering window's
                // top-level URL for partitioned storage, but their secure
                // context is inherited above rather than recomputed from a
                // Window-only top-level environment.
                true
            },
        }
    }
