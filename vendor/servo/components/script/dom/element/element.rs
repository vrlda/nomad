        ...
        V: Push<ApplicableDeclarationBlock>,
    {
        ...
        if let Some(rows) = rows {
            ...
            if rows > 0 {
                ...
                // `rows` supplies the textarea's default preferred block size; it is
                // not an authored definite `height`. Encoding it as `height` prevented
                // an auto-sized textarea from participating in flex/grid stretch and
                // made one-row controls overflow their own scrollport. Keep the row
                // metric as an intrinsic floor while leaving the preferred height auto.
                push(PropertyDeclaration::MinHeight(
                    specified::Size::LengthPercentage(NonNegative(
                        specified::LengthPercentage::Length(value),
                    )),
                ));
...
impl Element {
    ...
    fn handle_attribute_changes(
        ...
        // ARIA and native form-state changes are not necessarily text or child
        // mutations, but they change the platform accessibility node. Queue a
        // property refresh so live DOM changes are reflected without requiring
        // a full accessibility-tree rebuild.
        if namespace == ns!() && Self::accessibility_attribute(&name) {
            self.node
                .add_pending_accessibility_damage(AccessibilityDamage::Text);
        }
    ...
    fn accessibility_attribute(name: &LocalName) -> bool {
        matches!(
            *name,
            local_name!("aria-label")
                | local_name!("aria-hidden")
                | local_name!("aria-disabled")
                | local_name!("aria-required")
                | local_name!("aria-readonly")
                | local_name!("aria-expanded")
                | local_name!("aria-selected")
                | local_name!("aria-checked")
                | local_name!("aria-invalid")
                | local_name!("aria-valuenow")
                | local_name!("aria-valuemin")
                | local_name!("aria-valuemax")
                | local_name!("aria-level")
                | local_name!("role")
                | local_name!("checked")
                | local_name!("disabled")
                | local_name!("hidden")
...
impl ElementMethods<crate::DomTypeHolder> for Element {
    ...
    fn GetClientRects(&self, cx: &mut JSContext) -> DomRoot<DOMRectList> {
        let win = self.owner_window();
        let mut raw_rects = self.upcast::<Node>().border_boxes();
        // SVG child elements never get layout fragments (`svg > * { display: none }`
        // in servo.css), so their client geometry is derived from their SVG
        // geometry attributes instead; see `svg::geometry`.
        if raw_rects
            .iter()
            .all(|rect| rect.size.width == Au(0) && rect.size.height == Au(0))
        {
            raw_rects = geometry::client_rect_for_svg_element(self)
                .into_iter()
                .collect();
        }
    ...
    fn GetBoundingClientRect(&self, cx: &mut JSContext) -> DomRoot<DOMRect> {
        let win = self.owner_window();
        // As in `GetClientRects`, SVG child elements have no layout fragments,
        // so fall back to attribute-derived geometry when the fragment-based
        // query yields nothing or a zero rect.
        let rect = self
            .upcast::<Node>()
            .border_box()
            .filter(|rect| rect.size.width != Au(0) || rect.size.height != Au(0))
            .or_else(|| geometry::client_rect_for_svg_element(self))
            .unwrap_or_default();
...
impl VirtualMethods for Element {
    ...
    fn attribute_mutated(
        ...
        {
            node.dirty(cx.no_gc(), NodeDamage::ContentOrHeritage);
        }
