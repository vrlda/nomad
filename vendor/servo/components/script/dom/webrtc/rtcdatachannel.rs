...
impl RTCDataChannel {
    ...
    pub(crate) fn on_state_change(&self, cx: &mut JSContext, state: DataChannelState) {
        if let DataChannelState::Closed = state {
            self.on_close(cx);
            return;
        }
    ...
    fn send(&self, source: &SendSource) -> Fallible<()> {
        ...
        if let Err(error) =
            controller.send_data_channel_message(&self.get_servo_media_id(), message)
        {
            let message = match error {
                WebRtcError::Backend(message) => message,
            };
            return Err(Error::Operation(Some(message)));
        }
...
impl RTCDataChannelMethods<crate::DomTypeHolder> for RTCDataChannel {
    ...
    fn Close(&self) {
        ...
        if let Some(controller) = controller.as_ref() {
            controller.close_data_channel(&self.get_servo_media_id());
        }
    }
