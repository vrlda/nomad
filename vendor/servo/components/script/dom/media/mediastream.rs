...
use crate::dom::media::mediastreamtrackevent::MediaStreamTrackEvent;
...
impl MediaStream {
    ...
    pub(crate) fn new_single(
        ...
        Self::new_with_track(cx, global, &track)
    }
    ...
    fn queue_track_event(&self, track: &MediaStreamTrack, event_type: &'static str) {
        let this = Trusted::new(self);
        let track = Trusted::new(track);
        let event_type = Atom::from(event_type);
        self.global()
            .task_manager()
            .dom_manipulation_task_source()
            .queue(task!(media_stream_track_event: move |cx| {
                let this = this.root();
                let track = track.root();
                let event = MediaStreamTrackEvent::new(
                    cx,
...
impl MediaStreamMethods<crate::DomTypeHolder> for MediaStream {
    ...
    fn AddTrack(&self, track: &MediaStreamTrack) {
        ...
        self.queue_track_event(track, "addtrack");
    }
    ...
    fn RemoveTrack(&self, track: &MediaStreamTrack) {
        let removed = self
            ...
            .borrow()
            .iter()
            .any(|existing| *existing == track);
        if !removed {
            return;
        }
        self.tracks.borrow_mut().retain(|x| *x != track);
        self.queue_track_event(track, "removetrack");
    }
