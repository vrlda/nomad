...
use embedder_traits::{DownloadRequest, EmbedderMsg};
...
impl Activatable for HTMLAnchorElement {
    ...
    fn activation_behavior(
        ...
        if target.is::<HTMLImageElement>() {
            if let Some(element) = target.downcast::<Element>() {
                if element.has_attribute(&local_name!("ismap")) {
        ...
        if let Some(download_request) = self.download_request() {
            let document = self.owner_document();
            let window = document.window();
            window.send_to_embedder(EmbedderMsg::RequestDownload(
                window.webview_id(),
                download_request,
            ));
            return;
...
impl HTMLAnchorElement {
    fn download_request(&self) -> Option<DownloadRequest> {
        ...
        let url = self.owner_document().encoding_parse_a_url(&href).ok()?;
        let suggested_filename = element
            .get_attribute_string_value(&local_name!("download"))
