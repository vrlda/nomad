...
use aws_lc_rs::rsa::{KeyPair, KeyPairComponents, KeySize, PublicKeyComponents};
...
use crate::dom::cryptokey::{CryptoKey, Handle, KeyUsageVecHelper, RsaPrivateKey, RsaPublicKey};
...
pub(crate) fn generate_key(
    ...
    // AWS-LC generates RSA keys with the standards-recommended exponent 65537. Rejecting other
    // exponents here keeps generation on the constant-time, audited AWS-LC path; imported keys
    // may still use any odd exponent accepted by AWS-LC.
    ...
    if normalized_algorithm.public_exponent.as_slice() != [1, 0, 1] {
        return Err(Error::NotSupported(Some(
            "RSA key generation currently supports public exponent 65537 only".to_string(),
        )));
    }
    let key_size = match normalized_algorithm.modulus_length {
        ...
        _ => {
            return Err(Error::Operation(Some(
                "AWS-LC RSA key generation supports 2048, 3072, 4096, or 8192 bits".into(),
            )));
        },
    };
    let key_pair = KeyPair::generate(key_size)
        .map_err(|_| Error::Operation(Some("Failed to generate RSA private key".into())))?;
    let private_key = RsaPrivateKey::from_key_pair(&key_pair)
        .map_err(|_| Error::Operation(Some("Failed to serialize RSA private key".into())))?;
    let public_key = private_key
        .public_key()
        .map_err(|_| Error::Operation(Some("Failed to derive RSA public key".into())))?;
...
pub(crate) fn import_key(
    ...
    let (key_handle, key_type) = match format {
        ...
        KeyFormat::Spki => {
            ...
            let public_key = RsaPublicKey::from_spki_der(key_data).map_err(|_| {
                Error::Data(Some(
        ...
        KeyFormat::Jwk => {
            ...
            if jwk.d.is_some() {
                ...
                if jwk.oth.as_ref().is_some_and(|oth| !oth.is_empty()) {
                    return Err(Error::NotSupported(Some(
                        "Multi-prime RSA JWKs are not supported by AWS-LC".to_string(),
                    )));
                }
                ...
                // then throw a DataError.
                let private_key = private_key_from_components(&n, &e, &d, &p, &q, &dp, &dq, &qi)
                    .map_err(|_| {
                        Error::Data(Some(
            ...
            // Otherwise:
                ...
                let public_key = public_key_from_components(&n, &e).map_err(|_| {
                    Error::Data(Some(
    ...
    let (modulus_length, public_exponent) = match &key_handle {
        Handle::RsaPrivateKey(private_key) => rsa_key_metadata(private_key.as_bytes())?,
        Handle::RsaPublicKey(public_key) => rsa_key_metadata(public_key.as_bytes())?,
        _ => unreachable!(),
    };
    let algorithm = SubtleRsaHashedKeyAlgorithm {
        name: match &rsa_algorithm {
...
pub(crate) fn export_key(
    ...
    let result = match format {
        ...
        KeyFormat::Pkcs8 => {
            ...
            // Step 3.3. Let result be the result of DER-encoding data.
            ExportedKey::new_bytes(private_key.as_bytes().to_vec())
        },
        // If format is "jwk":
        KeyFormat::Jwk => {
            ...
            let (n, e) = match key.handle() {
                Handle::RsaPrivateKey(private_key) => rsa_public_components(private_key.as_bytes())
                    .map_err(|_| {
                        Error::Operation(Some("Failed to extract RSA components".into()))
                    })?,
                Handle::RsaPublicKey(public_key) => rsa_public_components(public_key.as_bytes())
                    .map_err(|_| {
                        Error::Operation(Some("Failed to extract RSA components".into()))
                    })?,
                _ => {
            ...
            jwk.encode_string_field(JwkStringField::N, &n);
            jwk.encode_string_field(JwkStringField::E, &e);
            ...
            if key.Type() == KeyType::Private {
                ...
                let components = rsa_private_components(private_key.as_bytes()).map_err(|_| {
                    Error::Operation(Some("Failed to extract RSA private components".into()))
                })?;
                jwk.encode_string_field(JwkStringField::D, &components.d);
                jwk.encode_string_field(JwkStringField::P, &components.p);
                jwk.encode_string_field(JwkStringField::Q, &components.q);
                jwk.encode_string_field(JwkStringField::DP, &components.dp);
                jwk.encode_string_field(JwkStringField::DQ, &components.dq);
                jwk.encode_string_field(JwkStringField::QI, &components.qi);
            }
...
impl<'a> DerReader<'a> {
    ...
    fn read(&mut self, expected_tag: u8) -> Result<&'a [u8], ()> {
        let tag = *self.bytes.get(self.offset).ok_or(())?;
        if tag != expected_tag {
            return Err(());
        }
        self.offset += 1;
        let first_length = *self.bytes.get(self.offset).ok_or(())?;
        self.offset += 1;
        let length = if first_length & 0x80 == 0 {
            ...
            let end = self.offset.checked_add(length_bytes).ok_or(())?;
            let bytes = self.bytes.get(self.offset..end).ok_or(())?;
            self.offset = end;
            bytes
                .iter()
                .try_fold(0usize, |length, byte| {
                    length.checked_mul(256)?.checked_add(*byte as usize)
                })
                .ok_or(())?
        };
...
fn sequence(bytes: &[u8]) -> Result<&[u8], ()> {
    let mut reader = DerReader::new(bytes);
    let value = reader.read(0x30)?;
    if reader.is_empty() {
        Ok(value)
    } else {
        Err(())
...
fn integer(bytes: &[u8]) -> Result<Vec<u8>, ()> {
    let mut reader = DerReader::new(bytes);
    let value = reader.read(0x02)?;
    if !reader.is_empty() || value.is_empty() || value[0] & 0x80 != 0 {
        return Err(());
    }
    ...
    Ok(value[first_nonzero..].to_vec())
}

fn rsa_public_components(bytes: &[u8]) -> Result<(Vec<u8>, Vec<u8>), ()> {
    let spki = sequence(bytes)?;
    let mut reader = DerReader::new(spki);
    let _algorithm = reader.read(0x30)?;
    let bit_string = reader.read(0x03)?;
    if !reader.is_empty() || bit_string.first() != Some(&0) {
        return Err(());
    }
    let rsa_key = sequence(&bit_string[1..])?;
    let mut reader = DerReader::new(rsa_key);
    let n = integer(reader.read(0x02)?)?;
    let e = integer(reader.read(0x02)?)?;
    if !reader.is_empty() {
        return Err(());
    }
    Ok((n, e))
...
fn rsa_private_components(bytes: &[u8]) -> Result<PrivateRsaComponents, ()> {
    let private_key_info = sequence(bytes)?;
    let mut reader = DerReader::new(private_key_info);
    let _version = reader.read(0x02)?;
    let _algorithm = reader.read(0x30)?;
    let private_key = sequence(reader.read(0x04)?)?;
    if !reader.is_empty() {
        return Err(());
    }
    let mut reader = DerReader::new(private_key);
    let _version = reader.read(0x02)?;
    let n = integer(reader.read(0x02)?)?;
    let e = integer(reader.read(0x02)?)?;
    let d = integer(reader.read(0x02)?)?;
    let p = integer(reader.read(0x02)?)?;
    let q = integer(reader.read(0x02)?)?;
    let dp = integer(reader.read(0x02)?)?;
    let dq = integer(reader.read(0x02)?)?;
    let qi = integer(reader.read(0x02)?)?;
    if !reader.is_empty() {
        return Err(());
    }
    Ok(PrivateRsaComponents {
        n,
        e,
        d,
        p,
...
fn rsa_key_metadata(bytes: &[u8]) -> Result<(u32, Vec<u8>), Error> {
    let (n, e) = rsa_public_components(bytes)
        .or_else(|_| rsa_private_components(bytes).map(|components| (components.n, components.e)))
        .map_err(|_| Error::Data(Some("Failed to parse RSA key components".into())))?;
    let first = *n
        .first()
        .ok_or_else(|| Error::Data(Some("RSA modulus is empty".into())))?;
    let leading_bits = 8 - first.leading_zeros();
    let modulus_length = ((n.len().saturating_sub(1) * 8) as u32) + leading_bits;
    Ok((modulus_length, e))
}
...
fn der_value(tag: u8, value: &[u8]) -> Vec<u8> {
...
fn der_integer(value: &[u8]) -> Vec<u8> {
    ...
    let mut integer = Vec::with_capacity(value.len() + usize::from(needs_leading_zero));
    if needs_leading_zero {
        integer.push(0);
    }
    integer.extend_from_slice(value);
    der_value(0x02, &integer)
}

fn der_sequence(values: &[Vec<u8>]) -> Vec<u8> {
    let length = values.iter().map(Vec::len).sum();
    let mut encoded = Vec::with_capacity(1 + der_length(length).len() + length);
    encoded.push(0x30);
    encoded.extend_from_slice(&der_length(length));
    for value in values {
        encoded.extend_from_slice(value);
    }
    encoded
...
fn public_key_from_components(n: &[u8], e: &[u8]) -> Result<RsaPublicKey, ()> {
    const RSA_ENCRYPTION_OID: &[u8] = &[
        0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x01,
    ];
    let rsa_key = der_sequence(&[der_integer(n), der_integer(e)]);
    let algorithm = der_sequence(&[RSA_ENCRYPTION_OID.to_vec(), der_value(0x05, &[])]);
    let mut bit_string = vec![0];
    bit_string.extend_from_slice(&rsa_key);
    let spki = der_sequence(&[algorithm, der_value(0x03, &bit_string)]);
    RsaPublicKey::from_spki_der(&spki)
}

fn private_key_from_components(
    n: &[u8],
    e: &[u8],
    d: &[u8],
    p: &[u8],
    ...
    let key_pair = KeyPair::from_components(&components).map_err(|_| ())?;
    RsaPrivateKey::from_key_pair(&key_pair)
}
