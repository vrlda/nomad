...
use aws_lc_rs::rsa::{KeyPair, PrivateDecryptingKey, PublicEncryptingKey};
...
/// Serialized RSA private-key material backed by AWS-LC for every private operation.
///
/// Keeping the DER representation in the DOM handle lets structured cloning remain cheap while
/// avoiding the vulnerable RustCrypto `rsa` implementation in the browser's WebCrypto path.
#[derive(Clone)]
pub(crate) struct RsaPrivateKey(Zeroizing<Vec<u8>>);

#[derive(Clone)]
pub(crate) struct RsaPublicKey(Vec<u8>);

impl RsaPrivateKey {
    pub(crate) fn from_pkcs8_der(bytes: &[u8]) -> Result<Self, ()> {
        PrivateDecryptingKey::from_pkcs8(bytes).map_err(|_| ())?;
        KeyPair::from_pkcs8(bytes).map_err(|_| ())?;
        Ok(Self(Zeroizing::new(bytes.to_vec())))
    }

    pub(crate) fn from_key_pair(key_pair: &KeyPair) -> Result<Self, ()> {
        let der = AsDer::<Pkcs8V1Der<'static>>::as_der(key_pair).map_err(|_| ())?;
        Self::from_pkcs8_der(der.as_ref())
    }
    ...
    pub(crate) fn public_key(&self) -> Result<RsaPublicKey, ()> {
        let private_key = PrivateDecryptingKey::from_pkcs8(self.as_bytes()).map_err(|_| ())?;
        let public_key = private_key.public_key();
        let der = AsDer::<PublicKeyX509Der<'static>>::as_der(&public_key).map_err(|_| ())?;
        RsaPublicKey::from_spki_der(der.as_ref())
    }
}

impl RsaPublicKey {
    pub(crate) fn from_spki_der(bytes: &[u8]) -> Result<Self, ()> {
        PublicEncryptingKey::from_der(bytes).map_err(|_| ())?;
...
impl MallocSizeOf for RsaPrivateKey {
    fn size_of(&self, ops: &mut malloc_size_of::MallocSizeOfOps) -> usize {
        self.0.size_of(ops)
    }
}

impl MallocSizeOf for RsaPublicKey {
    fn size_of(&self, ops: &mut malloc_size_of::MallocSizeOfOps) -> usize {
        self.0.size_of(ops)
    }
}
    ...
    fn try_from(value: SerializableCryptoKeyHandle) -> Result<Self, Self::Error> {
        match &value {
            ...
            SerializableCryptoKeyHandle::RsaPublicKey(public_key) => Ok(Handle::RsaPublicKey(
                RsaPublicKey::from_spki_der(public_key)?,
            )),
