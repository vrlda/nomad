...
use crate::timers::OneshotTimers;
...
pub(crate) struct DissimilarOriginWindow {
    ...
    /// The document exposed to an engine-managed isolated user-script world.
    /// Ordinary cross-origin windows leave this unset.
...
impl DissimilarOriginWindow {
    pub(crate) fn new(
        ...
        Self::new_with_document(cx, global_to_clone_from, window_proxy, None)
    }

    pub(crate) fn new_with_document(
        cx: &mut js::context::JSContext,
        global_to_clone_from: &GlobalScope,
        window_proxy: &WindowProxy,
        document: Option<&Document>,
    ) -> DomRoot<Self> {
        let pipeline_id = document
            .map(Document::pipeline_id)
            .unwrap_or_else(PipelineId::new);
        let win = Box::new(Self {
            ...
            document: MutNullableDom::new(document),
    ...
    pub(crate) fn event_loop_sender(&self) -> Option<ScriptEventLoopSender> {
        self.document()
            .map(|document| document.window().event_loop_sender())
    }
