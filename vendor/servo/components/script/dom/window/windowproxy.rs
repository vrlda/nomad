...
impl WindowProxy {
    ...
    /// Create the WindowProxy for an engine-managed isolated user-script
    /// world. Unlike a cross-origin proxy, this uses the normal forwarding
    /// traps against the isolated global so its expandos remain in that
    /// global while the associated Document is still the page Document.
    #[expect(unsafe_code)]
    pub(crate) fn new_isolated_world(
        cx: &mut JSContext,
        global_to_clone_from: &GlobalScope,
        page_window_proxy: &WindowProxy,
        document: &Document,
    ) -> DomRoot<DissimilarOriginWindow> {
        unsafe {
            let window_proxy = Box::new(WindowProxy::new_inherited(
                page_window_proxy.browsing_context_id(),
                page_window_proxy.webview_id(),
                Some(document.pipeline_id()),
                None,
                None,
                None,
                CreatorBrowsingContextInfo {
                    url: Some(document.url()),
                    origin: Some(document.origin().immutable().clone()),
                },
            ));

            let window = DissimilarOriginWindow::new_with_document(
                cx,
            ...
            // The JS proxy owns this allocation and releases it from its
            // finalizer, just like new_dissimilar_origin does.
            let _ = Box::into_raw(window_proxy);
            window
        }
    }
    ...
    fn create_auxiliary_browsing_context(
        ...
        load_data.storage_partition_key =
            document.global().storage_partition_key().map(str::to_owned);
