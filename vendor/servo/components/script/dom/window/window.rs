...
pub(crate) struct Window {
    ...
    /// Engine-managed isolated globals for embedder user scripts. Each
    /// stable world is retained for the lifetime of this Window so event
    /// listeners, promises, and extension state keep the same realm.
    #[ignore_malloc_size_of = "DOM-rooted isolated globals are accounted for by tracing"]
    user_script_worlds: DomRefCell<HashMap<String, Dom<DissimilarOriginWindow>>>,
...
impl Window {
    ...
    pub(crate) fn user_script_world(
        &self,
        cx: &mut JSContext,
        world_name: &str,
    ) -> DomRoot<DissimilarOriginWindow> {
        if let Some(world) = self.user_script_worlds.borrow().get(world_name) {
            return DomRoot::from_ref(&**world);
        }

        let window_proxy = self.window_proxy();
        let document = self.Document();
        let world =
            WindowProxy::new_isolated_world(cx, self.as_global_scope(), &window_proxy, &document);
        self.user_script_worlds
            .borrow_mut()
            .insert(world_name.to_owned(), Dom::from_ref(&*world));
        world
    }
...
impl Window {
    ...
    pub(crate) fn new(
        ...
        let win = Box::new(Self {
            ...
            paint_api,
            user_scripts,
            user_script_worlds: Default::default(),
            player_context,
