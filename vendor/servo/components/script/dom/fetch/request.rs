...
impl Request {
    ...
    pub(crate) fn from_net_request(
        ...
        *r.request.borrow_mut() = net_request;
        // Requests reconstructed from a network/service-worker request do not
        // pass through the DOM constructor below, but the Fetch API still
        // requires `signal` to be a live AbortSignal. Leaving it null makes a
        // worker-side `fetch()` panic when its handler reads request.signal.
        let signal = AbortSignal::create_dependent_abort_signal(cx, vec![], global);
        r.signal.set(Some(&signal));
        r
    }

    pub(crate) fn current_url(&self) -> ServoUrl {
        self.request.borrow().current_url()
    }
