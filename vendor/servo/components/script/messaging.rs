    ...
    pub(crate) fn pipeline_id(&self) -> Option<PipelineId> {
        match self {
            MixedMessage::FromConstellation(inner_msg) => match inner_msg {
                ...
                ScriptThreadMessage::ServiceWorkerPostMessage { pipeline_id, .. } => {
                    Some(*pipeline_id)
                },
                ScriptThreadMessage::ServiceWorkerControllerUpdate { pipeline_id, .. } => {
                    Some(*pipeline_id)
                },
                ...
                ScriptThreadMessage::AccessibilityAction(id, _) => Some(*id),
                ScriptThreadMessage::TriggerGarbageCollection => None,
                ScriptThreadMessage::CollectWebViewMemoryReport(..) => None,
            },
