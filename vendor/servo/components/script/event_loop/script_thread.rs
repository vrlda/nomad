...
use embedder_traits::user_contents::{
    UserContentManagerId, UserContents, UserScript, UserScriptWorld,
};
use embedder_traits::{
...
use crate::dom::node::{Node, NodeTraits, from_untrusted_node_address};
use crate::dom::servoparser::{ParserContext, ServoParser};
use crate::dom::types::DebuggerGlobalScope;
#[cfg(feature = "webgpu")]
use crate::dom::webgpu::identityhub::IdentityHub;
use crate::dom::window::Window;
use crate::dom::window::scrolling_box::ScrollAxisState;
...
impl ScriptThread {
    ...
    fn handle_msg_from_constellation(
        ...
        match msg {
            ...
            ScriptThreadMessage::ServiceWorkerPostMessage {
                pipeline_id,
                scope_url,
                script_url,
                worker_id,
                source_origin,
                data,
            } => self.handle_serviceworker_post_message_msg(
                cx,
                pipeline_id,
                scope_url,
                script_url,
            ...
            ScriptThreadMessage::ServiceWorkerControllerUpdate {
                ...
                worker_id,
                registration_id,
            } => self.handle_serviceworker_controller_update(
                cx,
                pipeline_id,
                scope_url,
                script_url,
            ...
            ScriptThreadMessage::AccessibilityAction(pipeline_id, request) => {
                self.handle_accessibility_action(pipeline_id, request, cx);
            },
            ScriptThreadMessage::TriggerGarbageCollection => unsafe {
                JS_GC(cx, GCReason::API);
            },
            ScriptThreadMessage::CollectWebViewMemoryReport(
                webview_id,
                pipeline_ids,
                reports_chan,
            ) => self.collect_reports_for_webview(cx, webview_id, &pipeline_ids, reports_chan),
        }
    }
    ...
    fn handle_msg_from_devtools(
        ...
        match msg {
            ...
            DevtoolScriptControlMsg::SetStyleSheetText(id, index, text, reply) => {
                devtools::handle_set_stylesheet_text(cx, &documents, id, index, text, reply);
            },
            ...
            DevtoolScriptControlMsg::GetStorage(id, reply) => {
                devtools::handle_get_storage(cx, &documents, id, reply)
            },
    ...
    fn collect_reports_for_webview(
        &self,
        cx: &mut js::context::JSContext,
        webview_id: WebViewId,
        ...
        let webview_prefix = format!("webview({webview_id:?})");

        perform_memory_report(|ops| {
            for pipeline_id in pipeline_ids {
                let Some(document) = documents.find_document(*pipeline_id) else {
                    continue;
                };
                ...
                document
                    ...
                    .layout()
                    .collect_reports(&mut reports, ops);
                for report in &mut reports[first_report..] {
                    report.path.insert(0, format!("pipeline({pipeline_id:?})"));
                    report.path.insert(0, webview_prefix.clone());
                }
            }

            reports.extend(get_reports(cx, webview_prefix, ops));
        });
    ...
    fn handle_serviceworker_post_message_msg(
        &self,
        cx: &mut js::context::JSContext,
        pipeline_id: PipelineId,
        scope_url: ServoUrl,
        ...
        let Some(window) = self.documents.borrow().find_window(pipeline_id) else {
            warn!("service-worker postMessage after target pipeline {pipeline_id} closed");
            return;
        };
        let container = window.Navigator(cx).ServiceWorker(cx);
        container.handle_message_from_worker(
            cx,
            data,
            worker_id,
            scope_url,
    ...
    fn handle_serviceworker_controller_update(
        &self,
        cx: &mut js::context::JSContext,
        pipeline_id: PipelineId,
        scope_url: ServoUrl,
        script_url: ServoUrl,
        worker_id: ServiceWorkerId,
        registration_id: servo_base::id::ServiceWorkerRegistrationId,
    ) {
        let Some(window) = self.documents.borrow().find_window(pipeline_id) else {
            warn!("service-worker controller update after target pipeline {pipeline_id} closed");
            return;
        };
        let container = window.Navigator(cx).ServiceWorker(cx);
        container.update_controller(
            cx,
            &scope_url,
            Some(worker_id),
            &script_url,
    ...
    fn ask_constellation_for_top_level_url(
        &self,
        sender_webview_id: WebViewId,
        sender_pipeline_id: PipelineId,
        browsing_context_id: BrowsingContextId,
        ...
        let (result_sender, result_receiver) = generic_channel::channel().unwrap();
        let msg = ScriptToConstellationMessage::GetTopLevelUrlForBrowsingContext(
            browsing_context_id,
            result_sender,
        );
        self.senders
        ...
        result_receiver
            .recv()
            .expect("Failed to get top-level URL from constellation.")
    }
    ...
    fn load(
        ...
        let window = Window::new(
            ...
            origin.clone(),
            final_url.clone(),
            if incomplete.parent_info.is_some() {
                self.ask_constellation_for_top_level_url(
                    incomplete.webview_id,
                    incomplete.pipeline_id,
            ...
            incomplete.load_data.storage_partition_key.clone(),
        ...
        // Resolve the initial service-worker controller against the final
        // document URL before document scripts can observe it. The Navigator
        // survives about:blank predecessors, so this must be scheduled after
        // each real document is installed.
        if pref!(dom_serviceworker_enabled) && matches!(final_url.scheme(), "http" | "https") {
            let navigator = window.Navigator(cx);
            navigator
                .ServiceWorker(cx)
                .schedule_initial_controller_match();
        }
    ...
    fn handle_accessibility_action(
        ...
        let Some(node_address) = document
            .window()
            .layout()
            .accessibility_node_address(request.target_tree, request.target_node)
        else {
            return;
        };
        ...
        match request.action {
            Action::Click => node.fire_synthetic_pointer_event_not_trusted(cx, atom!("click")),
            Action::Focus => {
            ...
            Action::Increment | Action::Decrement => {
                let Some(input) = node.downcast::<HTMLInputElement>() else {
                    return;
                };
                let result = if request.action == Action::Increment {
                    input.StepUp(cx, 1)
                } else {
                    input.StepDown(cx, 1)
                };
    ...
    fn handle_evaluate_javascript(
        ...
        let isolated_world = match &world {
            UserScriptWorld::Page => None,
            UserScriptWorld::Isolated(world_name) => Some(window.user_script_world(cx, world_name)),
        };
        let global_scope = isolated_world
            .as_ref()
            .map(|world| world.upcast::<GlobalScope>())
            .unwrap_or_else(|| window.as_global_scope());
        let mut realm = enter_auto_realm(cx, global_scope);
        let cx = &mut realm.current_realm();

        if let Some(world) = isolated_world.as_ref() {
            crate::dom::userscripts::install_document_binding(cx, global_scope, world);
        }
