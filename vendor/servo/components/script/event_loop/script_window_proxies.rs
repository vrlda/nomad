...
impl ScriptWindowProxies {
    ...
    pub(crate) fn remote_window_proxy(
        ...
        let (browsing_context_id, parent_pipeline_id, pipeline_opener) =
        ...
        // A top-level context carries its own opener recorded on the pipeline.
        // Without this, dissimilar-origin copies of auxiliary windows would
        // inherit the querying (often opener-less iframe) pipeline's opener
        // and wrongly report `window.opener === null`.
        let opener = if parent_pipeline_id.is_none() {
            pipeline_opener
        } else {
            opener
        };
