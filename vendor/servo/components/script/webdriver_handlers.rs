...
/// Compute the accessible name used by WebDriver's get computed label command.
///
/// This deliberately keeps the algorithm in the script process, next to the
/// DOM primitives it consumes. It covers the author-facing naming mechanisms
/// that are available in Servo today and, importantly, follows
/// `aria-labelledby` references recursively without allowing cycles.
pub(crate) fn handle_get_computed_label(
    ...
    let result = get_known_element(documents, pipeline, node_id).map(|element| {
        let mut visited = HashSet::new();
        let name = accessible_name(cx, &element, pipeline, &mut visited, false);
...
fn accessible_name(
    ...
    include_hidden_descendants: bool,
) -> String {
    let key = element.upcast::<Node>().unique_id(pipeline);
    if !visited.insert(key) {
        return String::new();
    }

    let document = element.owner_document();
    if let Some(labelled_by) = element.GetAttribute(cx, DOMString::from("aria-labelledby")) {
        let mut names = Vec::new();
        for id in labelled_by.str().split_ascii_whitespace() {
            let Some(label) = document.GetElementById(cx, DOMString::from(id)) else {
                continue;
            };
            let include_hidden = element_is_hidden(cx, &label);
            let name = accessible_name(cx, &label, pipeline, visited, include_hidden);
            if !name.is_empty() {
                names.push(name);
            }
        }
        if !names.is_empty() {
            return normalize_accessible_text(&names.join(" "));
        }
    }

    if let Some(label) = element.GetAttribute(cx, DOMString::from("aria-label")) {
        let label = normalize_accessible_text(&label.str());
        if !label.is_empty() {
            return label;
        }
    }

    let tag_name = element.local_name().as_ref();
    let host_language_name = match tag_name {
        "img" | "area" => element.GetAttribute(cx, DOMString::from("alt")),
        "input" | "textarea" | "select" => element.GetAttribute(cx, DOMString::from("placeholder")),
        _ => None,
    };
    if let Some(name) = host_language_name {
        let name = normalize_accessible_text(&name.str());
        if !name.is_empty() || tag_name == "img" || tag_name == "area" {
            return name;
        }
    }

    if matches!(tag_name, "input" | "textarea" | "select") {
        let title = element
            .GetAttribute(cx, DOMString::from("title"))
            .map(|title| normalize_accessible_text(&title.str()))
            .unwrap_or_default();
        if !title.is_empty() {
            return title;
        }
    ...
    let content = normalize_accessible_text(&visible_text_content(
        cx,
        element.upcast::<Node>(),
        include_hidden_descendants,
    ));
    if !content.is_empty() {
        return content;
    }
...
fn visible_text_content(cx: &mut JSContext, node: &Node, include_hidden: bool) -> String {
    ...
    for child in children {
        if let Some(element) = child.downcast::<Element>() {
            if !include_hidden && element_is_hidden(cx, element) {
                continue;
            }
        }
        content.push_str(&visible_text_content(cx, &child, include_hidden));
    }
    content
}

fn element_is_hidden(cx: &mut JSContext, element: &Element) -> bool {
    if element
        .GetAttribute(cx, DOMString::from("hidden"))
        .is_some()
    {
        return true;
    }
    if element
        .GetAttribute(cx, DOMString::from("aria-hidden"))
        .is_some_and(|value| value.str().eq_ignore_ascii_case("true"))
    {
        return true;
    }
    element
        .GetAttribute(cx, DOMString::from("style"))
        .is_some_and(|style| {
            style.str().split(';').any(|declaration| {
...
fn normalize_accessible_text(text: &str) -> String {
