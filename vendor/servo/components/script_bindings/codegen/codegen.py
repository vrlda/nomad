            ...
            if m.isAttr() and not m.isStatic() and m.type.isJSONType():
                ...
                ret += fill(
                    ...
                    if is_satisfied {
                      ...
                      if !JS_DefineProperty(cx, result, ${nameAsArray}, temp.handle(), JSPROP_ENUMERATE as u32) {
                        return false;
                      }
                    }
                    """,
        ...
        if aliasedMembers:
            ...
            def defineAliasesFor(m: IDLMethod) -> CGThing:
                return CGList([
                    CGGeneric(fill(
                        """
                        assert!(JS_GetProperty(cx, prototype.handle(), ${prop}, aliasedVal.handle_mut()));
                        """,
...
class CGMethodPromiseWrapper(CGAbstractExternMethod):
    ...
    def definition_body(self) -> CGThing:
        method_name = self.makeRustMethodName(self.method.identifier.name)
    ...
    def makeNativeName(descriptor: Descriptor, m: IDLMethod) -> str:
        return f"{CGMethodPromiseWrapper.makeRustMethodName(m.identifier.name)}_promise_wrapper"

    @staticmethod
    def makeRustMethodName(name: str) -> str:
...
let mut cx = JSContext::from_ptr(ptr::NonNull::new(cx).unwrap());
let cx = &mut cx;
JS_DefineProperty(cx, HandleObject::from_raw(obj), {name}, HandleValue::from_raw(args.get(0)), JSPROP_ENUMERATE as u32)
""")
...
if !expando.is_null() {
    ...
    if hasProp {
        return JS_ForwardGetPropertyTo(cx, expando.handle(), id, receiver, vp.reborrow());
    }
}"""
...
let mut vp = MutableHandle::from_raw(vp);
