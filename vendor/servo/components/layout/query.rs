...
fn containing_block_for_node<'a>(node: ServoLayoutNode<'a>) -> Option<ServoLayoutNode<'a>> {
    ...
    while let Some(ancestor) = unsafe { current_ancestor.dangerous_flat_tree_parent() } {
        // Always advance through the flat tree, including past nodes without a
        // layout box/style (for example display:contents and shadow-tree
        // plumbing). Leaving `current_ancestor` unchanged before `continue`
        // repeats the same parent forever. IntersectionObserver exercises this
        // query from the script thread, so that loop makes the entire document
