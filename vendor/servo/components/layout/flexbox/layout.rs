...
impl FlexContainerConfig {
    ...
    fn resolve_align_self_for_child(&self, child_style: &ComputedValues) -> AlignFlags {
        // `align-self: auto` first resolves to the parent's `align-items`. If that
        // value is itself `normal`, flex layout must then resolve `normal` to
        // `stretch`. `ComputedValuesExt::resolve_align_self` performs either
        // substitution in one pass, so an auto child of a normal container used to
        // incorrectly retain `normal` and skip cross-axis stretching.
        let resolved = child_style
            .resolve_align_self(self.align_items, AlignFlags::STRETCH)
            .0;
        let resolved = if resolved.value() == AlignFlags::NORMAL {
            AlignFlags::STRETCH | resolved.flags()
        } else {
            resolved
        };
        self.resolve_reversable_flex_alignment(resolved, self.flex_wrap_is_reversed)
    }
